...
Default (`--mode dispatch`): the same MAGI-2 layer is timed with its native
reference and default BF16 fast path, including routing and all preprocessing.
...
calls. The reference uses deterministic routed storage in diagnostic mode.
Use --mode dispatch to measure the
actual layer with the native and default fast paths, including the router. Unsupported
...
def parse_args(argv=None):
    ...
    parser.add_argument("--heads", type=int, default=3, help="EP4 local head count for MAGI-2 Preview.")
    parser.add_argument("--experts", type=int, default=256, help="Released experts per head.")
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="Use the portable deterministic native reference (needed by Triton-MUSA 3.2).",
    )
...
def env_switch(enabled):
    # Keep the legacy benchmark context API; dispatch is no longer opt-in.
    with patch.dict(os.environ, MAGI2_DETERMINISTIC="0"):
        yield
...
def measure(torch, calls, args):
    ...
    for index in range(len(names) * args.warmup + count):
        cache.zero_()
        cuda.synchronize()
        begin, end = cuda.Event(enable_timing=True), cuda.Event(enable_timing=True)
        begin.record()
        invoke()
        end.record()
        end.synchronize()
...
def check_and_capture(torch, moe, kernels, data, args):
    ...
    packed_w13 = moe._pack_bf16_w13(gate, up)
    route_buffers = moe._allocate_bf16_route_buffers(
        x.shape[0] * args.heads * args.top_k,
        args.heads * args.experts,
        128,
        x.device,
    )
    ...
    for result, reference in ((new_routes, sorted_routes[0]), (actual, expected)):
        ...
        errors.append(
            {"max_abs": (result.float() - reference.float()).abs().max().item(), "relative_l2": relative_l2.item()}
        )
...
def benchmark(torch, moe, kernels, tokens, args):
    ...
    packed_w13 = moe._pack_bf16_w13(gate, up)
    route_count = tokens * args.heads * args.top_k
    route_buffers = moe._allocate_bf16_route_buffers(route_count, args.heads * args.experts, 128, x.device)
    ...
    calls = {
        "reference_deterministic_routed": reference,
        "candidate_routed": lambda: moe._bf16_fused_moe_forward(
            x, probabilities, indices, packed_w13, down, route_buffers
        ),
    }
    if args.mode == "dispatch":
        config = moe.Magi2MultiHeadMoEConfig(
            args.heads * 256, args.heads, args.experts, args.top_k, 1280, torch.bfloat16
        )
        ...
        flat_x = x.reshape(tokens, -1)

        def native_dispatch():
            x_heads = flat_x.view(tokens, args.heads, 256)
            probs, idx = layer._route(x_heads)
            gather, weights, offsets = moe.global_sort_routes(probs, idx, args.experts)
            return moe.triton_mh_moe_forward(
                x_heads,
                gather,
                weights,
                offsets,
                ...
                layer.W_up,
                layer.W_down,
                deterministic=args.native_deterministic,
            ).reshape(tokens, -1)

        calls = {"native_dispatch": native_dispatch, "fused_dispatch": lambda: layer(flat_x)}
        torch.testing.assert_close(calls["fused_dispatch"](), calls["native_dispatch"](), rtol=2e-2, atol=0.5)
    timings = measure(torch, calls, args)
    names = list(calls)
    result = {"status": "pass", "tokens": tokens, "mode": args.mode, "correctness": errors, "timings": timings}
    result["speedup_p50_wall"] = (
        timings[names[0]]["synchronized_wall"]["p50_us"] / timings[names[1]]["synchronized_wall"]["p50_us"]
    )
    if args.stages:
        route_ids = (indices.int() + torch.arange(args.heads, device=x.device)[:, None, None] * args.experts).reshape(
            -1, args.top_k
        )
        ...
        result["standalone_stages_not_additive"] = measure(
            torch,
            {
                ...
                "candidate_pack_w13": lambda: torch.stack((gate.transpose(1, 2), up.transpose(1, 2)), dim=2).reshape(
                    args.heads * args.experts, 2560, 256
                ),
...
def main():
    ...
    report = {
        ...
        "sm_count": cuda.get_device_properties(0).multi_processor_count,
        "source_sha": sha,
        "module_path": moe.__file__,
        "torch": torch.__version__,
        "args": {**vars(args), "output": str(args.output)},
        "cache": "512 MiB flush outside each region",
        "capture": "eager; JIT warmed; no graph/compile",
        "results": [],
    }
