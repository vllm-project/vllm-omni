...
def prepare_deploy_configs(
    output_dir: Path,
    source_deploy: Path,
    max_model_len: int | None,
) -> dict[str, Path]:
    source = yaml.safe_load(source_deploy.read_text())
    config_dir = output_dir / "configs"
    config_dir.mkdir(parents=True, exist_ok=True)
    result: dict[str, Path] = {}
    for group in GROUPS.values():
        path = (config_dir / f"{group.deploy_name}.yaml").resolve()
        config = json.loads(json.dumps(source))
        for stage in config["stages"]:
            stage.pop("hbm_limit_gb", None)
        if max_model_len is not None:
            next(stage for stage in config["stages"] if stage["stage_id"] == 0)["max_model_len"] = max_model_len
...
def run_case(
    *,
    ...
    benchmark_json = (case_dir / "benchmark.json").resolve()
    ...
    if profile == "covo-chat":
        command = [
            "python",
            str(REPO / "benchmarks/hbm_admission/run_covo_chat_case.py"),
            "--model",
            model,
            "--num-prompts",
            str(num_prompts),
...
def aggregate(output_dir: Path, metadata: dict[str, Any]) -> dict[str, Any]:
    ...
    for group_name in GROUPS:
        for concurrency in metadata["concurrencies"]:
            ...
            peaks = [case.get("gpu", {}).get("memory_used_mib", {}).get("peak") for case in selected]
    ...
    for log_path in sorted((output_dir / "server_logs").glob("*.log")):
        ...
        summary["server_events"][log_path.name] = {
            name: len(pattern.findall(contents)) for name, pattern in event_patterns.items()
        }
    ...
    for new_group, baseline_group in comparisons:
        for concurrency in metadata["concurrencies"]:
            ...
            summary["comparisons_percent"][f"{new_group}_vs_{baseline_group}_c{concurrency}"] = deltas
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--profile", choices=("qwen3-tts", "covo-chat"), default="qwen3-tts")
...
def main() -> int:
    ...
    if args.summarize_only:
        ...
        return 0
    deploys = prepare_deploy_configs(
        args.output_dir,
        args.deploy_config,
        args.max_model_len,
    )
    ...
    metadata = {
        ...
        "source_deploy_config": str(args.deploy_config),
    ...
    for round_index in range(rounds):
        for group_name in orders[round_index]:
            ...
            expected_statuses = [
                args.output_dir / "cases" / f"round_{round_index + 1}_{group.name}_c{concurrency}" / "status.json"
                for concurrency in concurrencies
            ]
            if all(
                path.exists() and json.loads(path.read_text()).get("status") == "completed"
                for path in expected_statuses
            ):
                continue
