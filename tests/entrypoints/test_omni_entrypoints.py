...
class FakeAsyncOmniEngine:
    ...
    async def try_get_output_async(self) -> Any:
        # Mirror the real engine: park until a message is available instead
        # of returning None on empty. The AsyncOmni output loop no longer
        # polls (it relies on this awaitable suspending when the queue is
        # empty), so a synchronous None return would hot-spin the loop
