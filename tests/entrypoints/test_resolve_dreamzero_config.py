...
from vllm_omni.config.resolver import _load_stage_configs_from_model, resolve_model_config_path
...
def test_dreamzero_vla_resolves_to_dreamzero_config(monkeypatch):
    monkeypatch.setattr(
        "vllm_omni.config.resolver.get_config",
        lambda _model, trust_remote_code=True: type("Cfg", (), {"model_type": "vla"})(),
    )
    monkeypatch.setattr(
        "vllm_omni.config.resolver._looks_like_dreamzero",
        lambda _model: True,
    )
...
def test_dreamzero_config_sets_model_class_and_policy_config(monkeypatch):
    ...
    stage_configs, _ = _load_stage_configs_from_model(
