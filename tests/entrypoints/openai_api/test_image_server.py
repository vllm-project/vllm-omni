...
def test_image_edits_request_id_unique(async_omni_test_client):
    ...
    second-level precision. Two requests in the same second would get identical
    request_ids like 'img_edit_1234567890', causing request state collisions
    where one request's result would overwrite the other's.

    The fix uses random_uuid() to generate globally unique request IDs.
    This test verifies that two requests in the same second receive distinct
    request_ids (not relying on timing/concurrency to create uniqueness).
    """
    img_bytes_1 = make_test_image_bytes((16, 16))
    ...
    for request_id in captured_request_ids:
        assert request_id.startswith("img_edit_"), f"Expected request_id to start with 'img_edit-', got: {request_id}"
