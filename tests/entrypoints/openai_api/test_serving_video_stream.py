...
async def test_video_frame_selection_pairs_metadata_by_position(num_frames, frame_ids, repeat_image, expected_indices):
    """Queries wait for prewarmed PIL frames, with metadata paired by buffer position."""
    engine_prompts = []
    ...
    assert [part["type"] for part in image_parts] == ["image_pil"] * len(expected_indices)
    for part, index in zip(image_parts, expected_indices):
        raw = base64.b64decode(frames[index])
        assert part["image_pil"].tobytes() == Image.open(io.BytesIO(raw)).convert("RGB").tobytes()
        assert part["uuid"] == hashlib.md5(raw, usedforsecurity=False).hexdigest()
...
class PromptRecordingHandler(QwenOmniStreamingVideoHandler):
    ...
    def build_engine_prompt(
        self, config, frame_buffer, audio_buffer, message_history, query_text, prewarmed_frames, *, frame_indices=None
    ):
