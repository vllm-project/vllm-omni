...
class TestThinker2TalkerPDMode:
    ...
    def test_pd_matches_prefill_by_request_id_when_order_differs(self):
        """PD mode should match prefill outputs by request_id, not list position."""
        thinker2talker = self._import()
        dim = 16

        prompt_ids_0, output_ids_0 = _build_chat_token_ids(user_len=10, assistant_generated_len=3)
        prompt_ids_1, output_ids_1 = _build_chat_token_ids(user_len=8, assistant_generated_len=2)

        prefill_mm_0 = _make_multimodal_output(
            embed=torch.full((len(prompt_ids_0), dim), 11.0),
            hidden=torch.full((len(prompt_ids_0), dim), 21.0),
        )
        prefill_mm_1 = _make_multimodal_output(
            embed=torch.full((len(prompt_ids_1), dim), -11.0),
            hidden=torch.full((len(prompt_ids_1), dim), -21.0),
        )
        decode_mm_0 = _make_multimodal_output(
            embed=torch.full((len(output_ids_0), dim), 1.0),
            hidden=torch.full((len(output_ids_0), dim), 2.0),
        )
        decode_mm_1 = _make_multimodal_output(
            embed=torch.full((len(output_ids_1), dim), -1.0),
            hidden=torch.full((len(output_ids_1), dim), -2.0),
        )
        ...
        prefill_stage = _FakeStage(
            ...
            engine_outputs=[
                _FakeRequestOutput(
                    request_id="req-1",
                    prompt_token_ids=prompt_ids_1,
                    outputs=[_FakeCompletionOutput(token_ids=[output_ids_1[0]], multimodal_output=prefill_mm_1)],
                ),
                _FakeRequestOutput(
                    request_id="req-0",
                    prompt_token_ids=prompt_ids_0,
                    outputs=[_FakeCompletionOutput(token_ids=[output_ids_0[0]], multimodal_output=prefill_mm_0)],
                ),
            ],
        )
        decode_stage = _FakeStage(
            ...
            engine_outputs=[
                _FakeRequestOutput(
                    request_id="req-0",
                    prompt_token_ids=prompt_ids_0,
                    outputs=[_FakeCompletionOutput(token_ids=output_ids_0, multimodal_output=decode_mm_0)],
                ),
                _FakeRequestOutput(
                    request_id="req-1",
                    prompt_token_ids=prompt_ids_1,
                    outputs=[_FakeCompletionOutput(token_ids=output_ids_1, multimodal_output=decode_mm_1)],
                ),
            ],
        )
        ...
        info_0 = results[0]["additional_information"]
        info_1 = results[1]["additional_information"]
        assert torch.equal(info_0["thinker_prefill_embeddings"][: len(prompt_ids_0)], prefill_mm_0[_EMBED_LAYER_KEY])
        assert torch.equal(info_1["thinker_prefill_embeddings"][: len(prompt_ids_1)], prefill_mm_1[_EMBED_LAYER_KEY])
