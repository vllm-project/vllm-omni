...
across consecutive tokens and active-batch sizes. Capture failure must
fall back to eager permanently. Warmup must not consume RNG.
"""
...
def test_diffusion_graph_startup_warmup_captures_all_batches_without_consuming_rng() -> None:
    ...
    assert set(executor._entries) == {(1, 10, 1.3), (2, 10, 1.3), (3, 10, 1.3), (4, 10, 1.3)}
...
def test_diffusion_graph_capture_failure_falls_back_to_eager(
    ...
    with torch.inference_mode():
        ...
        strict_executor = VibeVoiceDiffusionGraphExecutor(sampler, head, capture_failure_fatal=True)
        ...
        with pytest.raises(RuntimeError, match="Required VibeVoice diffusion CUDA-graph capture failed"):
            strict_executor.sample(positive, negative, noise, guidance_scale=1.3, num_inference_steps=10)
