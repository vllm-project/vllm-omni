...
from vllm_omni.model_executor.models.minicpmo_4_5.minicpmo_4_5_omni_tts import (
    ...
    _batched_codec_sampling_enabled,
    _max_audio_tokens,
...
def _make_talker() -> MiniCPMO45OmniTTSForConditionalGeneration:
    ...
    talker._batched_codec_sampling = True
    return talker
...
def test_batched_repetition_penalty_matches_rows_across_chunks(mocker) -> None:
    ...
    vocab_size = 11
    logits = (
        torch.arange(batch_size * vocab_size, dtype=torch.float32).reshape(batch_size, vocab_size)
        - 100
    )
    histories = [
        torch.tensor(
            [(row + offset) % vocab_size for offset in range(row % 7)],
            dtype=torch.long,
        )
...
def test_batched_codec_sampling_switch_rejects_invalid_values(monkeypatch) -> None:
    monkeypatch.setenv("VLLM_OMNI_MINICPMO45_BATCHED_CODEC_SAMPLING", "sometimes")

    with pytest.raises(ValueError, match="VLLM_OMNI_MINICPMO45_BATCHED_CODEC_SAMPLING"):
        _batched_codec_sampling_enabled()


def test_batched_codec_sampling_switch_defaults_on_and_allows_rollback(monkeypatch) -> None:
    monkeypatch.delenv("VLLM_OMNI_MINICPMO45_BATCHED_CODEC_SAMPLING", raising=False)
    assert _batched_codec_sampling_enabled() is True

    monkeypatch.setenv("VLLM_OMNI_MINICPMO45_BATCHED_CODEC_SAMPLING", "0")
    assert _batched_codec_sampling_enabled() is False


def test_talker_reuses_initialized_scheduler_sampler() -> None:
    talker = _make_talker()
    sentinel = object()
    calls = []

    def sampler(logits, metadata):
        calls.append((logits, metadata))
        return sentinel

    talker._sampler = sampler
    logits = torch.ones(1, 2)
    metadata = object()

    assert talker.sample(logits, metadata) is sentinel
...
def test_talker_codec_sampling_rollback_keeps_request_local_calls(mocker) -> None:
    talker = _make_talker()
    talker._batched_codec_sampling = False

    def sample(hidden, histories, request_ids, steps):
        assert hidden.shape == (1, 2)
        assert len(histories) == len(request_ids) == len(steps) == 1
        return torch.tensor([2 if request_ids[0] == "req-a" else 3])

    request_local_sample = mocker.patch.object(
        talker,
        "_sample_audio_codes",
        side_effect=sample,
    )
    ...
    assert request_local_sample.call_count == 2
    assert _routed(output, 0)["codes"]["audio"].tolist() == [[2]]
    assert _routed(output, 1)["codes"]["audio"].tolist() == [[3]]
