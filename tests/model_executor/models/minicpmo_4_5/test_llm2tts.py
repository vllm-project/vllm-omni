...
class TestBasicShape:
    def test_request_level_multimodal_output_feeds_orchestrator_bridge(self) -> None:
        ...
        assert torch.equal(
            unpack_tts_hidden(out[0]["model_intermediate_buffer"]["hidden_states"]["tts"]), hidden[1:]
        )
    ...
    def test_strict_talker_receives_canonical_nested_handoff(self) -> None:
        ...
        assert torch.equal(unpack_tts_hidden(buffer["hidden_states"]["tts"]), hidden[2:4])
    ...
    def test_latent_in_multimodal_output_takes_precedence(self) -> None:
        ...
        assert torch.equal(unpack_tts_hidden(buffer["hidden_states"]["tts"]), latent[3:4].to(torch.float32))
...
class TestTtsRegionDetection:
    ...
    def test_4_5_markers_detected(self) -> None:
        ...
        assert torch.equal(unpack_tts_hidden(buffer["hidden_states"]["tts"]), hidden[3:5])
    ...
    def test_plain_chat_without_tts_markers_uses_assistant_span(self) -> None:
        ...
        assert torch.equal(unpack_tts_hidden(buffer["hidden_states"]["tts"]), hidden[2:5])
