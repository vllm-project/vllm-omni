...
class _ModelConfigView:
    """Minimal ModelConfig stand-in exposing only what ``_falcon_config_view``
    reads/writes (``hf_config`` / ``hf_text_config``); a real ``VllmConfig`` is
    GPU-bound to build at CPU tier."""
...
class _VllmConfigView:
    def __init__(self, model_config):
        self.model_config = model_config


def test_01b_declares_hybrid_mamba_interfaces():
    """Regression for the two blockers the metadata-only tests missed: the Slow
    AR must declare the hybrid-Mamba interfaces (or vLLM never sizes the Mamba2
    state cache -> ``assert mamba_block_size is not None``) and must override the
    three state classmethods rather than inherit the Qwen2 parent's absence."""
    cls = Audio8TTS01BSlowARForConditionalGeneration
    assert cls.is_hybrid is True
    assert cls.has_inner_state is True
    for name in (
        "get_mamba_state_shape_from_config",
        "get_mamba_state_dtype_from_config",
        "get_mamba_state_copy_func",
    ):
        assert name in vars(cls), f"{name} must be overridden on the 0.1b class"


def test_01b_falcon_config_view_exposes_backbone_fields():
    """Regression for blocker #1: vLLM's FalconH1Model (and its mamba-state
    classmethods) read ``model_config.hf_config`` directly, so the Slow AR must
    hand them a view whose ``hf_config`` is the FalconH1 text_config carrying
    ``vocab_size`` -- the exact attribute whose absence crashed model init."""
    from transformers.models.falcon_h1.configuration_falcon_h1 import FalconH1Config

    outer = _config_01b()
    assert not hasattr(outer, "vocab_size") or "vocab_size" not in outer.__dict__
    view = _falcon_config_view(_VllmConfigView(_ModelConfigView(outer)))
    tc = view.model_config.hf_config
    assert isinstance(tc, FalconH1Config)
    assert tc.vocab_size == 69633
    assert view.model_config.hf_text_config is tc
