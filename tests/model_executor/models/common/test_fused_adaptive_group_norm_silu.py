...
# split = min(ceil(waves * SMs / groups), ceil(spatial / 4096)). Every case above
# is at most 64x64 = 4096 positions, so ceil(spatial / 4096) = 1 keeps all of
# them on the unsplit path on any device. The cases below pin the environment so
# the split path is exercised deterministically, and assert that premise.
...
def _modulation(batch, channels, dtype):
    """Conditioning signals shaped the way the DiT ResBlock supplies them."""
    scale = torch.randn(batch, channels, device="cuda", dtype=dtype) * 0.1
    shift = torch.randn(batch, channels, device="cuda", dtype=dtype) * 0.1
    return scale, shift


def _eager_adagn(x, weight, bias, scale, shift, num_groups=32, eps=1e-6):
    """fp32 reference, with the modulation broadcast over the spatial axes."""
    bc = (x.shape[0], x.shape[1]) + (1,) * (x.ndim - 2)
    normed = F.group_norm(x.float(), num_groups, weight.float(), bias.float(), eps)
    return F.silu(normed * (1.0 + scale.float().reshape(bc)) + shift.float().reshape(bc))
...
def test_split_reduction_matches_eager(channels, hw, dtype, monkeypatch):
    ...
    scale, shift = _modulation(1, channels, dtype)
    ...
    fused_out = fused_adaptive_group_norm_silu(x, weight, bias, scale, shift, 32, 1e-6)
    ref_out = _eager_adagn(x, weight, bias, scale, shift).to(dtype)
...
def test_split_reduction_ragged_tail(monkeypatch):
    ...
    scale, shift = _modulation(1, C, torch.float32)
    ...
    fused_out = fused_adaptive_group_norm_silu(x, weight, bias, scale, shift, 32, 1e-6)
    ref_out = _eager_adagn(x, weight, bias, scale, shift)
...
def test_split_reduction_batched(monkeypatch):
    ...
    is always 0: a wrong batch offset, workspace rows indexed by group instead of
    by (batch, group), or ``scale``/``shift`` read from the wrong sample would pass
    every one of them. The samples get different scales and offsets so that
    normalizing one with the other's statistics is far outside tolerance, and the
    spatial size is the ragged one from above.
    """
    monkeypatch.setenv(_SPLIT_WAVES_ENV, "8")
    torch.manual_seed(0)
    B, C, H, W = 2, 64, 96, 130
    ...
    weight = torch.randn(C, device="cuda")
    bias = torch.randn(C, device="cuda")
    scale, shift = _modulation(B, C, torch.float32)
...
def test_split_reduction_large_offset(monkeypatch):
    ...
    fused_out = fused_adaptive_group_norm_silu(x, weight, bias, scale, shift, 32, 1e-6)
    bc = (1, C, 1, 1)
    normed = F.group_norm(x.double(), 32, weight.double(), bias.double(), 1e-6)
    ref_out = F.silu(normed * (1.0 + scale.double().reshape(bc)) + shift.double().reshape(bc)).float()
...
def test_split_is_a_perf_knob_only(monkeypatch):
    ...
    unsplit = fused_adaptive_group_norm_silu(x, weight, bias, scale, shift, 32, 1e-6)

    monkeypatch.setenv(_SPLIT_WAVES_ENV, "8")
    assert _split_of(x) > 1, "premise: waves=8 must enable the split"
    heavily_split = fused_adaptive_group_norm_silu(x, weight, bias, scale, shift, 32, 1e-6)
...
def test_split_reduction_is_deterministic(monkeypatch):
    ...
    args = (x, weight, bias, scale, shift, 32, 1e-6)
    first = fused_adaptive_group_norm_silu(*args)  # also warms autotune
    reference = fused_adaptive_group_norm_silu(*args)
    assert torch.equal(first, reference), "autotune warm-up should have settled by the second call"
    for _ in range(4):
        assert torch.equal(fused_adaptive_group_norm_silu(*args), reference)
