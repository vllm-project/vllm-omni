...
def test_atomic_add(dtype):
    ...
    available = cuda.is_available() or (hasattr(torch, "musa") and torch.musa.is_available())
    if not available:
        pytest.skip("No CUDA/MUSA device")
    if dtype == torch.bfloat16 and getattr(torch.version, "musa", None) is not None:
        if tuple(int(part) for part in triton.__version__.split(".")[:2]) < (3, 6):
