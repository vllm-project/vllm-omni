...
@pytest.mark.parametrize("container_attrs", [[], ["other_blocks"]])
def test_regionally_compile_can_override_offload_containers(monkeypatch, container_attrs):
    model = _ModelWithWrappedRepeatedBlocks()
    model._regional_compile_blocks_attrs = container_attrs
    compile_calls = []
    ...
    expected = list(model.other_blocks) if container_attrs else []
    assert compile_calls == expected
    assert model._layerwise_offload_blocks_attrs == ["transformer_blocks"]
