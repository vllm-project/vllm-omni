...
def clear_mhc_failure_cache():
    """Keep the process-lifetime production cache isolated between tests."""
...
@pytest.mark.cpu
def test_model_wiring_uses_fused_eager_path(monkeypatch):
    calls = []
...
@pytest.mark.cpu
def test_no_triton_uses_native_fallback(monkeypatch):
    monkeypatch.setattr(mhc, "HAS_TRITON", False)
    args = coefficients(tokens=2)
    actual = mhc.MHCPostResidual().forward_cuda(*args, scale=0.125, out_dtype=torch.float32)
    for got, expected in zip(actual, reference(args)):
        torch.testing.assert_close(got, expected, rtol=0, atol=0)

    mix_args = mix_inputs(tokens=2)
    torch.testing.assert_close(mhc.MHCMix().forward_cuda(*mix_args), mix_reference(mix_args), rtol=0, atol=0)


@pytest.mark.cpu
@pytest.mark.parametrize(
    ("device_type", "native_required"),
    [("cpu", True), ("meta", True), ("hip", True), ("xpu", True), ("cuda", False), ("musa", False)],
)
def test_unvalidated_backend_uses_native(device_type, native_required):
    tensor = SimpleNamespace(device=SimpleNamespace(type=device_type), requires_grad=False)
    assert mhc._native_required((tensor,)) is native_required


@pytest.mark.cpu
def test_kernel_failure_is_cached_and_falls_back(monkeypatch):
    monkeypatch.setattr(mhc, "HAS_TRITON", True)
    monkeypatch.setattr(mhc, "_native_required", lambda tensors: False)
    warnings = []
    monkeypatch.setattr(mhc.logger, "warning", lambda *args, **kwargs: warnings.append((args, kwargs)))
    ...
    monkeypatch.setattr(mhc, "_mhc_post_residual_kernel", Broken())
    args = tuple(x.contiguous() for x in coefficients(tokens=2))
    op = mhc.MHCPostResidual()
    expected = reference(args)
    for _ in range(2):
        actual = op.forward_cuda(*args, scale=0.125, out_dtype=torch.float32)
        for got, want in zip(actual, expected):
            torch.testing.assert_close(got, want, rtol=0, atol=0)
    assert ("cpu", 0, "post_residual") in mhc._FAILED_MHC_KERNELS
    assert len(warnings) == 1
    assert "post_residual" in str(warnings[0])


@pytest.mark.cpu
def test_kernel_oom_is_propagated_without_caching(monkeypatch):
    monkeypatch.setattr(mhc, "HAS_TRITON", True)
    monkeypatch.setattr(mhc, "_native_required", lambda tensors: False)
    ...
    monkeypatch.setattr(mhc, "_mhc_post_residual_kernel", OutOfMemory())
    args = tuple(x.contiguous() for x in coefficients(tokens=2))
    with pytest.raises(RuntimeError, match="out of memory"):
        mhc.MHCPostResidual().forward_cuda(*args, scale=0.125, out_dtype=torch.float32)
    assert ("cpu", 0, "post_residual") not in mhc._FAILED_MHC_KERNELS
