...
from vllm_omni.diffusion.distributed.vae_parallel_state import generate_contiguous_rank_groups
...
def test_group_creation_failure_cleans_partial_state_for_retry(monkeypatch):
    local_group = object()
    calls = 0
    destroyed = []
    ...
    def successful_new_group(*, ranks, backend):
        group = (tuple(ranks), backend)
        retry_groups.append(group)
        return group

    monkeypatch.setattr(vae_parallel_state.dist, "new_group", successful_new_group)
    vae_parallel_state.initialize_vae_parallel_group(2, backend="gloo")

    assert retry_groups == [((0, 1), "gloo"), ((2, 3), "gloo")]
    assert vae_parallel_state.get_vae_group() == ((0, 1), "gloo")
    assert vae_parallel_state.get_vae_group_ranks() == [0, 1]
