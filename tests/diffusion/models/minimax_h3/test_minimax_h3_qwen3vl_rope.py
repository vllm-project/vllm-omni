...
def test_qwen3_vl_npu_rope_matches_bsnd_reference(
    num_q_heads: int,
    num_kv_heads: int,
    seq_len: int,
) -> None:
    ...
    q = torch.randn(1, seq_len, num_q_heads, 128, device=device, dtype=dtype)
    k = torch.randn(1, seq_len, num_kv_heads, 128, device=device, dtype=dtype)
    ...
    cos_bsnd = cos.unsqueeze(2)
    sin_bsnd = sin.unsqueeze(2)
    ...
    torch.testing.assert_close(
        q_out,
        q * cos_bsnd + rotate_half(q) * sin_bsnd,
        atol=2e-2,
        rtol=2e-2,
    )
    torch.testing.assert_close(
        k_out,
        k * cos_bsnd + rotate_half(k) * sin_bsnd,
        atol=2e-2,
        rtol=2e-2,
    )
...
def test_qwen3_vl_npu_rope_preserves_batched_mrope_positions() -> None:
    """Keep per-sample Qwen3-VL M-RoPE frequencies on the BSND NPU path."""
    ...
    q = torch.randn(batch_size, seq_len, 8, head_dim, device=device, dtype=dtype)
    k = torch.randn(batch_size, seq_len, 2, head_dim, device=device, dtype=dtype)
