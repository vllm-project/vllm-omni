...
def _torch_invoke(a, b, c, weights, sorted_ids, expert_ids, num_padded, *, top_k, config, fuse_swiglu):
    ...
    live_rows = int(num_padded.item())
    live_blocks = live_rows // block_size
    assert sorted_ids.numel() >= live_rows
    assert expert_ids.numel() * block_size == sorted_ids.numel()
    written = []
    for block, expert in enumerate(expert_ids[:live_blocks].tolist()):
...
def test_align_bf16_routes_preserves_routes_and_pads_expert_blocks(route_ids):
    ...
    live_rows = int(num_padded.item())
    assert sorted_ids.numel() >= live_rows
    valid_ids = sorted_ids[:live_rows][sorted_ids[:live_rows] < ids.numel()].long()
    assert sorted(valid_ids.tolist()) == list(range(ids.numel()))
    for block, expert in enumerate(expert_ids[:expected_blocks].tolist()):
...
def test_bf16_forward_head_offsets_and_interleaved_weights(monkeypatch, num_tokens, num_heads):
    ...
    x, probabilities, indices, gate, up, down = inputs
    actual = moe._bf16_fused_moe_forward(x, probabilities, indices, moe._pack_bf16_w13(gate, up), down)
...
@pytest.mark.parametrize(
    ...
    [
        ("cuda", torch.bfloat16, None, 1, False, "triton"),
        ("cuda", torch.bfloat16, "0", 4096, False, "bf16"),
        ("cuda", torch.bfloat16, "1", 4096, False, "triton"),
        ("cuda", torch.float16, None, 4096, False, "triton"),
        ("cuda", torch.float32, None, 4096, False, "triton"),
        ("cuda", torch.bfloat16, "0", 1, True, "bf16"),
        ("cpu", torch.bfloat16, None, 1, False, "torch"),
    ],
)
def test_local_forward_dispatch_contract(
    ...
    if device_type == "cpu":
        x_heads = torch.randn(num_tokens, 1, 4, dtype=dtype)
    else:
    ...
    monkeypatch.setattr(layer, "_get_bf16_route_buffers", Mock(return_value=route_buffers))
...
def test_local_forward_fast_path_propagates_kernel_failure(monkeypatch):
    ...
    x_heads = SimpleNamespace(
        ...
        is_cuda=True,
        # The fused route is selected for CUDA/MUSA only once the token count
        # reaches the production fast-path threshold.
        shape=(4096, 1, 4),
    )
    probabilities = torch.tensor([[[1.0]]])
    indices = torch.tensor([[[0]]])
    ...
    monkeypatch.setattr(moe, "global_sort_routes", sort)
    monkeypatch.setattr(moe, "_bf16_fused_moe_forward", fast)
    monkeypatch.setattr(layer, "_get_bf16_route_buffers", Mock(return_value=object()))
    monkeypatch.setattr(moe, "torch_mh_moe_forward", torch_kernel)
    monkeypatch.setattr(moe, "triton_mh_moe_forward", triton_kernel)
    monkeypatch.delenv("MAGI2_DETERMINISTIC", raising=False)
...
@pytest.mark.cpu
def test_bf16_weight_and_route_caches_are_reused_and_invalidated():
    config = moe.Magi2MultiHeadMoEConfig(
        hidden_size=4,
        num_heads=1,
        num_experts=2,
    ...
    layer = moe.Magi2MultiHeadMoE(config, ep_group=Magi2ParallelGroup(None, 1, 0))
    with torch.no_grad():
        layer.W_gate.fill_(0.25)
        layer.W_up.fill_(-0.5)

    first = layer._get_bf16_packed_w13()
    assert layer._get_bf16_packed_w13() is first
    with torch.no_grad():
        layer.W_gate.add_(1)
    second = layer._get_bf16_packed_w13()
    assert second is not first
    torch.testing.assert_close(second, moe._pack_bf16_w13(layer.W_gate, layer.W_up))

    buffers = layer._get_bf16_route_buffers(8, layer.W_gate.device)
    assert layer._get_bf16_route_buffers(8, layer.W_gate.device) is buffers
    assert layer._get_bf16_route_buffers(257, layer.W_gate.device) is not buffers
...
def test_bf16_reference_atomic_add_capability(device_type):
    ...
    device = _gpu_device(device_type)
    if device_type == "musa":
        version = tuple(int(part) for part in fused_moe_kernels.triton.__version__.split(".")[:2])
        if version < (3, 6):
            pytest.skip("Triton-MUSA < 3.6 does not support BF16 atomic_add")
    inputs = _moe_inputs(
        num_tokens=2,
        num_heads=1,
        hidden_size=64,
    ...
    actual = moe.triton_mh_moe_forward(x, gather_ids, sorted_probs, offsets, gate, up, down, deterministic=False)
    deterministic = moe.triton_mh_moe_forward(x, gather_ids, sorted_probs, offsets, gate, up, down, deterministic=True)
    assert torch.isfinite(actual).all()
