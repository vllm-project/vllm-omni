...
class TestFlux2TransformerWeightLoading:
    ...
    def test_weight_loading_tp2(self, setup_tp_group):
        ...
        mock_weights.append(
            ("transformer_blocks.0.attn.add_kv_proj.weight", add_kv_proj_weight)
        )

        # 3. Test single block weight loading
        single_block_qkv_mlp_weight = torch.randn(
            18432 + 18432 * 3, 6144
        )  # Full weights
        mock_weights.append(
            (
                "single_transformer_blocks.0.attn.to_qkv_mlp_proj.weight",
                single_block_qkv_mlp_weight,
            )
        )
        ...
        assert model.transformer_blocks[0].attn.to_qkv.weight.shape == (
            9216,
            6144,
...
class TestFlux2RopePositionEmbedding:
    ...
    def test_rope_position_embedding(self):
        ...
        # Verify value range
        assert torch.all(freqs_cos >= -1) and torch.all(
            freqs_cos <= 1
        ), "cos values should be in [-1, 1]"
        assert torch.all(freqs_sin >= -1) and torch.all(
            freqs_sin <= 1
        ...
        assert torch.allclose(
            cos_sq_sin_sq, torch.ones_like(cos_sq_sin_sq), atol=1e-6
        ), "cos^2 + sin^2 should ≈ 1"
        ...
        assert torch.allclose(
            freqs_cos, freqs_cos_same
        ), "Same positions should produce same embeddings"
        assert torch.allclose(
            freqs_sin, freqs_sin_same
        ), "Same positions should produce same embeddings"
...
class TestFlux2PackedModuleMapping:
    ...
    def test_packed_module_mapping(self, setup_tp_group):
        ...
        assert (
            attn_block.to_qkv.weight.shape == expected_qkv_shape
        ...
        assert (
            attn_block.add_kv_proj.weight.shape == expected_add_kv_shape
    ...
    def test_packed_mapping_edge_cases(self, setup_tp_group):
        ...
        assert any(
            "to_qkv" in p for p in loaded_params
        ), "to_q should be mapped to to_qkv"
