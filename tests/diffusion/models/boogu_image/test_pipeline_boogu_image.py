...
def test_forward_batched_ti2i_double_guidance_cfg_parallel_matches_sequential(monkeypatch):
    ...
    req = _wrap_request_batch(
        [
            (edit_prompt("make it winter", "ugly", 0.25), _sampling(**kw, generator=torch.Generator().manual_seed(1))),
            (
                edit_prompt("a dog in a park", "blurry", 0.75),
...
def test_forward_batched_ti2i_variable_length_partner_padding(monkeypatch):
    ...
    A = edit_prompt("make it winter", "ugly", 0.25)
    ...
    short_b = edit_prompt("a dog", "blurry", 0.75)
    long_b = edit_prompt("a dog running through a large green park full of tall autumn trees", "blurry", 0.75)
    long_neg_b = edit_prompt(
