...
class TestLTX23ImageToVideoPipeline:
    ...
    def test_ltx23_i2v_denoised_latent_step_accepts_base_loop_step_index(self):
        from vllm_omni.diffusion.models.ltx2_3.pipeline_ltx2_3_image2video import LTX23ImageToVideoPipeline
        ...
        def fake_scheduler_step_maybe_with_cfg(noise_pred, t, latents, do_true_cfg, per_request_scheduler):
            seen["noise_pred"] = noise_pred
            seen["t"] = t
            seen["latents"] = latents
            seen["do_true_cfg"] = do_true_cfg
            seen["per_request_scheduler"] = per_request_scheduler
            return latents[0] + noise_pred[0], latents[1] + noise_pred[1]

        pipe.scheduler_step_maybe_with_cfg = fake_scheduler_step_maybe_with_cfg

        video_latents = torch.zeros(1, 2, 3)
        audio_latents = torch.zeros(1, 4, 5)
        noise_pred_video = torch.ones_like(video_latents)
        noise_pred_audio = torch.ones_like(audio_latents)
        scheduler = object()
        out = pipe._step_denoised_latents(
            SimpleNamespace(video_audio_scheduler=scheduler),
            SimpleNamespace(latents=video_latents, audio_latents=audio_latents),
            noise_pred_video,
            noise_pred_audio,
            torch.tensor(0.5),
            0,
        ...
        torch.testing.assert_close(out[0], video_latents + noise_pred_video)
        torch.testing.assert_close(out[1], audio_latents + noise_pred_audio)
        assert seen["do_true_cfg"] is False
        assert seen["per_request_scheduler"] is scheduler
...
class TestLTX23ImageToVideoModule:
    ...
    def test_post_process_func_importable(self):
        ...
        from vllm_omni.diffusion.models.ltx2_3.pipeline_ltx2_3_image2video import get_ltx2_post_process_func
    ...
    def test_i2v_class_matches_package_export(self):
        ...
        from vllm_omni.diffusion.models.ltx2_3 import LTX23ImageToVideoPipeline as PackageExported
