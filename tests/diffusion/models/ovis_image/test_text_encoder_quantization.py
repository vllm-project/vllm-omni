...
from vllm_omni.diffusion.models.ovis_image import quantization
...
def encoder():
    ...
    return Qwen3Model(config).to(dtype=torch.bfloat16).eval()
...
def test_encoder_requires_component_opt_in(encoder, config):
    ...
    assert quantization.prepare_ovis_text_encoder_fp8(encoder, config, torch.device("cpu")) == 0
...
def test_encoder_rejects_unsupported_fp8_before_mutation(encoder, config):
    ...
    with pytest.raises(ValueError, match="dynamic online FP8"):
        quantization.prepare_ovis_text_encoder_fp8(
