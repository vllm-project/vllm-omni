...
class _FakeStageConfig:
    def __init__(self, config_dict: dict[str, Any]):
        ...
        # [yurain] runtime block mirrors YAML; pd_utils reads runtime.env.VLLM_MOONCAKE_BOOTSTRAP_PORT
        self.runtime = config_dict.get("runtime", {}) or {}
...
class TestValidatePDConfig:
    ...
    def test_valid_config_passes(self, monkeypatch):
        ...
        # [yurain] no error = validation passed
        assert omni._pd_separation_pair == (0, 1)
...
class TestGetPDConnectorInfo:
    ...
    def test_extracts_bootstrap_addr_for_mooncake(self, monkeypatch):
        ...
        # [yurain] MooncakeConnector expects remote_bootstrap_addr with http:// scheme
        assert info["prefill_bootstrap_addr"] == "http://127.0.0.1:25201"
        assert info["prefill_bootstrap_port"] == 25201
        assert info["prefill_bootstrap_host"] == "127.0.0.1"

    def test_runtime_env_overrides_extra_config_port(self, monkeypatch):
        """runtime.env.VLLM_MOONCAKE_BOOTSTRAP_PORT must win when set.

        vllm's MooncakeConnector reads VLLM_MOONCAKE_BOOTSTRAP_PORT from
        the worker process env at startup, so the orchestrator's view of
        the bootstrap address MUST match it. When both runtime.env and
        kv_connector_extra_config carry the same value the validator is
        happy; this test exercises the runtime.env-only path.
        """
        prefill_cfg = _prefill_stage_cfg()
        prefill_cfg["runtime"] = {"env": {"VLLM_MOONCAKE_BOOTSTRAP_PORT": 25301}}
        # [yurain] drop extra-config port; runtime.env is the only source
        prefill_cfg["engine_args"]["kv_transfer_config"]["kv_connector_extra_config"] = {}
        ...
        info = omni._pd_connector_info
        assert info["prefill_bootstrap_addr"] == "http://127.0.0.1:25301"
        assert info["prefill_bootstrap_port"] == 25301

    def test_mismatched_env_and_extra_config_port_raises(self, monkeypatch):
        """Inconsistent runtime.env vs extra-config port must fail validation.

        Otherwise vllm would listen on the env value while the
        orchestrator hands decode an addr derived from the extra-config
        value, and KV pulls would fail with a confusing connection error.
        """
        prefill_cfg = _prefill_stage_cfg()
        prefill_cfg["runtime"] = {"env": {"VLLM_MOONCAKE_BOOTSTRAP_PORT": 25301}}
        # [yurain] extra-config 25201 vs runtime.env 25202 -> mismatch
...
class TestSamplingParamsAutoDuplication:
    ...
    def test_auto_duplicates_for_4_stage_pipeline(self, monkeypatch):
        ...
        # [yurain] simulate outputs for all stages
        ...
        # [yurain] 3 params for 4 stages -> auto-duplicate
...
class TestPDRouting:
    ...
    def test_prefill_stage_receives_max_tokens_1(self, monkeypatch):
        ...
        # [yurain] put stage outputs in both queues
        ...
        # [yurain] check stage 0 input queue (skip stage_ready message)
...
# ===================================================================
# Non-destructive regression (M-P-N-D migration)
# ===================================================================


class TestNonDestructiveRegression:
    """Verify that the M-P-N-D migration does not regress non-PD / 1P1D paths.

    These tests cover the post-migration acceptance criteria:
      * Non-PD deployments must produce empty PD containers and NOT emit any
        PD-info log line (requirement 8.1).
      * Importing ``vllm_omni`` without ``mooncake-transfer-engine`` installed
        must not log ERROR (requirement 8.2).
      * 1P1D legacy callers (``_get_pd_separation_pair``) must continue to
        observe a tuple identical to the pre-migration behaviour
        (requirement 3.5 / 5.5).
    """

    def test_non_pd_pipeline_zero_side_effect(self, monkeypatch, caplog):
        """Stages without is_prefill_only / is_decode_only ⇒ all PD containers empty."""
        # ``_talker_stage_cfg`` / ``_code2wav_stage_cfg`` deliberately omit
        # ``is_prefill_only`` and ``is_decode_only``, matching a non-PD
        # pipeline (e.g. plain text generation).
        plain_stage_a = _talker_stage_cfg(stage_id=0, engine_input_source=[])
        plain_stage_b = _code2wav_stage_cfg(stage_id=1, engine_input_source=[0])

        with caplog.at_level("INFO", logger="vllm_omni.entrypoints.pd_utils"):
            omni = _make_pd_omni(monkeypatch, [plain_stage_a, plain_stage_b])

        assert omni._get_pd_separation_pair() is None
        assert omni._get_pd_prefill_ids() == []
        assert omni._get_pd_decode_ids() == []
        assert omni._pd_decode_id is None
        assert omni._pd_decode_to_prefill == {}
        assert omni._pd_connector_info is None
        assert omni._pd_connector_info_by_prefill == {}
        assert omni._pd_kv_params_by_req == {}
        assert omni._pd_prefill_inflight == {}
        # No "PD disaggregation detected" / "M-P-N-D PD topology" log emitted.
        joined = "\n".join(rec.getMessage() for rec in caplog.records)
        assert "PD disaggregation detected" not in joined
        assert "M-P-N-D PD topology detected" not in joined

    def test_1p1d_legacy_pair_equivalent(self, monkeypatch):
        """K=1 ∧ N=1 ⇒ ``_get_pd_separation_pair`` returns ``(p, d)`` exactly."""
        omni = _make_pd_omni(
            monkeypatch,
            [
                _prefill_stage_cfg(),
        ...
        # Legacy 1P1D callers must keep working:
        pair = omni._get_pd_separation_pair()
        assert pair == (0, 1)
        # And the new M-P-N-D views collapse to single-element lists:
        assert omni._get_pd_prefill_ids() == [0]
        assert omni._get_pd_decode_ids() == [1]
        assert omni._get_pd_decode_id() == 1

    def test_mooncake_missing_no_error_log(self, monkeypatch, caplog):
        """If mooncake-transfer-engine is not importable, no ERROR should leak."""
        # Force the importer used by both pd_utils._init_pd_state and patch.py
        # to behave as if mooncake is missing.
        from vllm_omni.distributed.kv_transfer import mooncake_pd_patch as _mp

        monkeypatch.setattr(_mp, "_import_mooncake_module", lambda: None)
        # Reset internal _patched flag so apply_* re-runs the missing path.
        monkeypatch.setattr(_mp, "_patched", False, raising=False)

        with pytest.raises(ImportError):
            _mp.apply_mooncake_connector_patch()
        # Acceptable: WARNING about "Cannot import MooncakeConnector".
        # Forbidden: any ERROR-level log.
        for rec in caplog.records:
            assert rec.levelname != "ERROR", f"Unexpected ERROR log when mooncake missing: {rec.getMessage()}"
