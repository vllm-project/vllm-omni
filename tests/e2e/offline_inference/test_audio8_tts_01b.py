...
"""E2E offline tests for Audio8 TTS Preview 0.1b (Falcon-H1 hybrid DualAR).

Same DualAR protocol and 44.1 kHz codec as the 0.6b variant (text-only prompts
are plain token ids, voice-clone prompts are a placeholder of the exact final
length); only the Stage 0 slow backbone differs (Falcon-H1 Mamba2 + attention
hybrid instead of Qwen2). These tests exercise the Falcon-H1 weight-loading and
...
# Distinct inputs so a leaked per-request codec buffer (two requests decoding to
# the same audio) fails the pairwise-difference assertion.
CONCURRENT_TEXTS = [
...
# vllm-omni owns the `arktts` config (backbone-aware); remote code stays off so
# transformers does not prefer the checkpoint's auto_map over the Falcon-H1 view.
...
# At ``core_model`` the harness patches stages to ``load_format: dummy``, so the
# AR runs on random weights, never emits EOS, and generates to ``max_tokens``.
# Duration bounds are meaningful only with real weights.
...
def test_text_to_audio_concurrent_002(omni_runner: OmniRunner, run_level: str) -> None:
    ...
    Per-request codec state is keyed by request id; distinct inputs plus a
    pairwise-difference assertion catch leaked codec state.
    """
