...
- **Acceptance criteria:** `bash scripts/serve_fun_audio_chat.sh` + `bash scripts/test_fun_audio_chat_curl.sh` produces a wav where (a) `usage.completion_tokens > 0`, (b) duration ∈ [1 s, 30 s], (c) Whisper transcription is non-empty. No `Orchestrator thread crashed` in the server log after the 200 OK.
...
- **Next actions (in order):**
  1. Log-instrument Stage-0's `forward` / `compute_logits` / `postprocess` in `fun_audio_chat.py` to confirm they are (or aren't) being called, and with what `input_ids.shape`. This alone disambiguates root causes 1/2 from 3/4.
  ...
  3. If candidate 1 is confirmed, either patch the chat template (tracked, model-weights-side) or teach our processor to *always* inject `<|audio_bos|><|AUDIO|>×N<|audio_eos|>` into the prompt at whatever position the user's `input_audio` part sat.
  4. Regardless of root cause 3: add the `prompt_token_ids=[]` guard in `vllm_omni/model_executor/stage_input_processors/funaudiochat.py::talker2code2wav` so Stage 1 never tries to schedule an empty prompt (defensive).
- **Affected paths:** `vllm_omni/model_executor/models/fun_audio_chat/fun_audio_chat.py` (processor), `vllm_omni/model_executor/stage_input_processors/funaudiochat.py` (empty-CRQ guard), possibly `vllm_omni/model_executor/models/fun_audio_chat/pipeline.py` (engine_output_type semantics).
- **Non-goals:** do not touch the three parity-verified modules (`encoder.py`, `crq_decoder.py`, module-side `forward`) for this item — they are correct, the standalone driver proves it.
