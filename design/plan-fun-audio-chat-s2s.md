...
### O12. Serve path produces 0 tokens — trace and fix the OpenAI → two-stage wiring
- **Goal:** make `vllm-omni serve` actually invoke Stage-0 forward + CRQ + Stage-1 token2wav on a real `/v1/chat/completions` request, producing the same clean audio the standalone driver (`scripts/run_native_infer_s2s.py`, commit `0c963af3`) already proves is achievable from the ported modules.
- **Observed symptom on `ck7vv9ag.wav` (server log `/tmp/vllm-omni-serve.log`, request `chatcmpl-9cfa21592b4e1ada`, 2026-04-22):**
  - Server returns `200 OK` with a well-formed `ChatCompletion`, but `message.content = null`, `usage.completion_tokens = 0`, `usage.prompt_tokens = 0`, `finish_reason = stop`.
  - `message.audio.data` decodes to a 40 ms silent wav — exactly the `empty_out` fallback we emit from `FunAudioChatToken2Wav.forward` when `input_ids.numel() == 0`.
  - Stage 0 fires in the orchestrator (`_handle_add_request: stage=0 … prompt_type=OmniEngineCoreRequest`) but produces no text and no CRQ tokens.
  - Stage 1 input processor logs `funaudiochat talker2code2wav: no crq_tokens in stage 0 output`, passes an empty token list to Stage 1's engine.
  - Stage 1's vllm scheduler then crashes: `assert total_num_scheduled_tokens > 0` at `vllm/v1/worker/gpu_model_runner.py:1798` because it scheduled `{'chatcmpl-…': -1}` tokens for an empty prompt. This kills `StageEngineCoreProc`, the orchestrator observes `EngineDeadError`, and the API server logs `RuntimeError: Orchestrator thread crashed` after the response has already been flushed.
  - Other signal: `WARNING … Invalid output modality: text, ignoring it` during orchestrator `_handle_add_request`. The request asked for `modalities: ["text", "audio"]`; only audio is being honored.
- **Four candidate root causes (investigate in order, cheapest first):**
  1. *Chat-template / placeholder substitution never happens.* `FunAudioChatMultiModalProcessor._get_prompt_updates` builds `PromptReplacement(modality="audio", target=[audio_token_id], ...)`. The checkpoint's `chat_template.jinja` may emit `<|AUDIO|>` only on the *text* side of the user content, but the vllm chat renderer is passing `input_audio` as a plain modality without first expanding it into `<|AUDIO|>` placeholders in the rendered prompt. If the tokenized prompt has **no** `<|AUDIO|>` token at all, `_get_prompt_updates` is a no-op and the engine sees a prompt that's ≤ 1 token long (explaining `prompt_tokens=0`, `num_computed_tokens=1`).
  2. *`modalities` output config not wired to our two-stage pipeline.* The `modalities: ["text", "audio"]` request-level field is what tells the OpenAI layer to attach `message.audio` to the response. The `Invalid output modality: text` warning indicates vllm-omni's output-modality routing is choosing an empty set; text tokens may be suppressed as a side-effect, leaving the model with nothing to decode.
  3. *Engine-output-type mismatch between stages.* `pipeline.py` sets Stage 0 `engine_output_type="latent"` but the orchestrator routes `chatcmpl-…` with `prompt_type=OmniEngineCoreRequest`. If the scheduler treats `latent` as "nothing to decode" when there are no audio features on the decode side, it could finish the request at 0 tokens. Cross-check against another two-stage LLM_AR→LLM_GENERATION pipeline that's known to work in serve (e.g. cosyvoice3).
  4. *Stage 1 input processor assumes non-empty CRQ.* Even with Stage 0 fixed, `talker2code2wav` must not return a `prompt_token_ids=[]` when CRQ is empty — that triggers the `-1 scheduled` assertion. Either short-circuit in the orchestrator when Stage-0 produced no CRQ tokens, or emit a 1-token sentinel that the Stage-1 model short-circuits to silence inside its own forward.
- **Non-negotiable deployment rule (from adversarial-review v5):** every fix lands in **tracked model / pipeline / processor / stage-input code** under `vllm_omni/model_executor/models/fun_audio_chat/` or `vllm_omni/model_executor/stage_input_processors/funaudiochat.py`. The `scripts/serve_fun_audio_chat.sh` wrapper must become purely cosmetic — it should only pick `--host`, `--port`, and `--stage-configs-path`, and must *not* carry any serve-critical knob (env var, CLI arg, PYTHONPATH, or env-dependent config) without which the pipeline fails to generate tokens. Plain `vllm-omni serve <ckpt> --omni --stage-configs-path vllm_omni/deploy/funaudiochat.yaml` must work end-to-end without a wrapper.
- **Acceptance criteria:**
  1. *Functional:* starting the server via the bare `vllm-omni serve` command above, then POSTing `/v1/chat/completions` with a user `input_audio` part, produces a response with `usage.completion_tokens > 0`, non-null `message.content`, `message.audio.data` that decodes to a wav of duration ∈ [1 s, 30 s]. No `Orchestrator thread crashed` in the server log after the 200 OK.
  2. *ASR sanity:* Whisper transcription of the returned wav is non-empty and has ≥ 1 CJK or ASCII letter character.
  3. *Streaming:* the streaming variant (`"stream": true`) yields at least one non-empty `delta.content` chunk and emits audio chunks; the final accumulated `message.audio.data` matches the non-streaming path within perceptual tolerance.
  4. *Regression guard:* any HTTP 200 on `/v1/chat/completions` with `usage.completion_tokens == 0` must be treated as a server-side error — either raise a 500, or emit a structured `logger.error("0-token completion", extra={...})` that the CI test asserts against (choose whichever vllm-omni's error-taxonomy already uses for engine-side failures).
- **Automated CI coverage (not a shell script):** add `tests/e2e/online_serving/test_fun_audio_chat_chat_completions.py` that:
  1. Uses the existing vllm-omni online-serving test fixture pattern (grep `tests/e2e/online_serving/test_voxcpm*` / `test_cosyvoice3_tts*` for the conventions) to boot `vllm-omni serve` via the bare CLI in a subprocess, then tear it down.
  2. Posts the request body that `scripts/test_fun_audio_chat_curl.sh` sends today (keep that script around as a manual smoke harness; it is not a regression barrier).
  3. Asserts each of the four acceptance criteria above, both for the non-streaming and streaming variants.
  4. Skips cleanly when the checkpoint or `fun-audio-chat-ref` extra isn't installed (env-gated fixture, same convention as `tests/e2e/offline_inference/test_cosyvoice3.py`).
- **Dependencies:** O6 (per-request state), O7 (Stage-1 bridge), O8 (prompt count) — all landed in serve-path module (`fun_audio_chat.py` commit `88e3c070`, `a7e48771`, `3e599b8d`). The bug is *above* the model in vllm-omni's serve/orchestrator/stage-input-processor stack, not in the ported modules themselves.
- **Diagnostic actions (in order, cheapest first):**
  1. Log-instrument Stage-0's `forward` / `compute_logits` / `postprocess` in `fun_audio_chat.py` (temporary, removed before merge) to confirm they are (or aren't) being called, and with what `input_ids.shape`. This alone disambiguates root causes 1/2 from 3/4.
  2. Compare `FunAudioChatMultiModalProcessor._call_hf_processor` / `_get_prompt_updates` against a working serve-path reference — cosyvoice3 is the closest analog since it also has LLM_AR → token2wav two-stage with audio I/O. Likely delta: cosyvoice3's processor explicitly inserts `<|AUDIO|>` into the chat-rendered prompt, whereas ours relies on the chat_template to do it.
  3. If candidate 1 is confirmed, fix landing options, in preference order:
     a. *Adjust our processor* (ideal) to handle the `input_audio` content block directly — insert `<|audio_bos|><|AUDIO|>×N<|audio_eos|>` into the rendered prompt at the position of the user's audio part, mirroring what other two-stage audio-input models do. Lives entirely in `FunAudioChatMultiModalProcessor`.
     b. *Ship a correct `chat_template.jinja`* — tracked under `vllm_omni/model_executor/models/fun_audio_chat/` and wired via `pipeline.py` / yaml as the default chat template for this `model_arch`, so operators don't have to pass `--chat-template` at `vllm-omni serve`.
     c. *Do NOT* rely on a user-supplied `--chat-template` flag — that recreates the deployment-skew failure mode the adversarial review flagged.
  4. Defensive: `talker2code2wav` must not hand an empty token list to Stage 1. On empty CRQ: short-circuit by returning an `OmniTokensPrompt` carrying a single sentinel token whose Stage-1 `forward` detects and returns an empty-audio `OmniOutput` gracefully — so the Stage-1 scheduler never sees `num_scheduled_tokens == 0`. Cover with a unit test of `talker2code2wav` against an empty-`crq_tokens` stage-0 output.
- **Affected paths:**
  - `vllm_omni/model_executor/models/fun_audio_chat/fun_audio_chat.py` (processor — candidate 3a).
  - `vllm_omni/model_executor/models/fun_audio_chat/chat_template.jinja` (new, optional — candidate 3b).
  - `vllm_omni/model_executor/models/fun_audio_chat/pipeline.py` (if the chat template needs to be wired into stage config).
  - `vllm_omni/model_executor/stage_input_processors/funaudiochat.py` (empty-CRQ defensive guard).
  - `tests/e2e/online_serving/test_fun_audio_chat_chat_completions.py` (new, the CI regression barrier).
  - Possibly `vllm_omni/engine/async_omni_engine.py` or an nearby hook for the 0-token-completion server-side guard (kept minimal, mirrors vllm-omni's existing engine-error taxonomy).
- **Non-goals:** do not touch the three parity-verified modules (`encoder.py`, `crq_decoder.py`, module-side `forward`/`compute_logits`/`postprocess`) for this item — they are correct, the standalone driver proves it. Also do not put any serve-critical config into `scripts/serve_fun_audio_chat.sh` — that's the precise anti-pattern adversarial-review v5 flagged.

### O13. Stage-1 audio never reaches `message.audio` in the chat-completion response
- **Goal:** the `/v1/chat/completions` response carries the synthesized audio under `message.audio.data` (base64 wav), matching OpenAI chat modalities semantics, with `transcript` set to the Stage-0 generated text.
- **Observed state on `ck7vv9ag.wav` (server log `/tmp/vllm-omni-serve.log`, request `chatcmpl-9bbb9d5f63f2408e`, 2026-04-22 02:10):**
  - **Stage 0 generates text and CRQ cleanly.** `usage.completion_tokens=512`, `message.content` is coherent English ("Yes, gentle instrumental music...").
  - **Stage 1 vocoder runs end-to-end.** Log line `FunAudioChatToken2Wav.forward: synthesised wav shape=(1, 446400) sr=24000` = 18.6 s of audio at 24 kHz, the right length for 485 CRQ tokens.
  - **The response body has `"audio": null`.** Raw OpenAI chat-completion payload decodes to `message.audio = null`, `usage.completion_tokens = 512`, no `transcript`, no `data`. The audio bytes never cross from `OmniOutput.multimodal_outputs["audio"]` (what Stage-1 returns) into `ChatCompletionMessage.audio`.
- **Candidate root causes (investigate in order):**
  1. *OpenAI-chat response assembler ignores Stage-1 `multimodal_outputs["audio"]`.* `vllm_omni/entrypoints/openai/serving_chat.py` is the surface where the orchestrator's per-stage outputs get merged into a single `ChatCompletionMessage`. The text path (Stage 0 → `message.content`) obviously works. The audio branch likely checks for a specific key/shape that we don't emit. Likely fix: align our Stage-1 `OmniOutput.multimodal_outputs` key/shape with what serving_chat expects (`"audio_output"` vs `"audio"`, tensor vs bytes vs `{data, format}` dict, list-of-per-req vs flat tensor).
  2. *`pipeline.py` doesn't declare Stage 1's output as a `message.audio`-shaped final output.* We set `final_output=True, final_output_type="audio"` on Stage 1, but maybe the chat-completion path needs additional metadata (e.g. `output_modality`, `response_field="audio"`, a specific connector).
  3. *Missing `audio.transcript` wiring.* OpenAI's chat-audio response shape expects `message.audio = {"id", "data" (base64), "expires_at", "transcript"}`. Even if our multimodal_outputs["audio"] is the raw wav tensor, something needs to base64-encode it and attach the Stage-0 text as the transcript. This logic likely lives in vllm-omni's OpenAI serializer, not in the model code, but it may require a structured `multimodal_outputs["audio"]` entry (e.g. a dict, not a plain tensor list).
  4. *`modalities=["text","audio"]` is being interpreted as text-only due to the earlier warning* (`Invalid output modality: text, ignoring it`). If the orchestrator's routing skips the audio output entirely because it thinks only text was requested, Stage-1 runs but its output is discarded in the response assembler.
- **Acceptance criteria:**
  1. A POST to `/v1/chat/completions` with `modalities:["text","audio"]` returns a 200 with `message.audio.data` non-empty, decodes to a wav that Whisper transcribes to non-empty speech, and `message.audio.transcript` equals (or closely matches) `message.content`.
  2. `usage.completion_tokens > 0` and `message.content` is non-null (text path still works).
  3. No warning `Invalid output modality: text, ignoring it` in the server log for requests with `modalities:["text","audio"]`.
  4. The tracked e2e test from O12 (`tests/e2e/online_serving/test_fun_audio_chat_chat_completions.py`) asserts all three.
- **Diagnostic actions (cheapest first):**
  1. Grep `vllm_omni/entrypoints/openai/serving_chat.py` and any shared response-assembler for how `multimodal_outputs["audio"]` is consumed. Compare against how VoxCPM's `/v1/audio/speech` TTS path handles audio bytes for a hint at the expected shape (even though the endpoint differs, the response-assembly interior often shares code paths).
  2. Log the exact `OmniOutput` shape that the orchestrator hands to the serving layer just before response serialization. If `multimodal_outputs` is empty or missing the audio key by then, the loss happens between Stage-1 forward and orchestrator aggregation (stage input processor, OmniOutput propagation).
  3. If the serializer expects `multimodal_outputs["audio"]` to be `list[bytes]` (encoded wav) instead of `list[Tensor]`, adapt our Stage-1 `forward` to base64-encode or hand back a pre-serialized wav bytestring. Keep the tensor available for tests via a sidecar key if needed.
  4. If #1 shows the chat-response path never reads audio from Stage-1 outputs at all (i.e. the only OpenAI path for audio is `/v1/audio/speech`), bias toward making the *chat* route accept a Stage-1 audio sidecar OR split the deploy into a single-stage `/v1/chat/completions` (text-only) plus a separate TTS endpoint — **but only as a last resort**, since it contradicts the Fun-Audio-Chat contract (audio AND text in one turn).
- **Dependencies:** O12 (pipeline / processor / compute_logits fixes — all landed on commits up to `52cc…`).
- **Non-goals:** do not touch the parity-verified modules (`encoder.py`, `crq_decoder.py`) or the Stage-1 vocoder itself (`token2wav.py`'s `_run_token2wav` — we've confirmed it produces valid wavs). This is purely about surfacing Stage-1 output into the HTTP response shape.
- **Affected paths (expected):**
  - `vllm_omni/entrypoints/openai/serving_chat.py` (response assembly)
  - Possibly `vllm_omni/model_executor/models/fun_audio_chat/token2wav.py` (OmniOutput shape tweak)
  - Possibly `vllm_omni/entrypoints/async_omni.py` or `omni_base.py` (multimodal output propagation)
  - `tests/e2e/online_serving/test_fun_audio_chat_chat_completions.py` (add the three assertions above)
...
- 2026-04-22 v5 — standalone E2E works (commit `0c963af3`, Whisper-verified); vllm-omni serve wiring exposed a new integration bug on first real request. Added **O12** with concrete log evidence (`chatcmpl-9cfa21592b4e1ada`), four candidate root causes in cost order, and diagnostic-first action list. O12 is code-only and scoped strictly to serve/orchestrator/stage-input wiring — must not touch the parity-verified modules.
- 2026-04-22 v6 — addressed adversarial-review v5 on O12:
  1. [high] *Fix must not live in the wrapper script.* O12 now carries a "non-negotiable deployment rule": plain `vllm-omni serve <ckpt> --omni --stage-configs-path vllm_omni/deploy/funaudiochat.yaml` must work. `scripts/serve_fun_audio_chat.sh` is cosmetic-only and must not carry serve-critical knobs. Fix candidates ranked: processor patch (3a) > tracked `chat_template.jinja` wired via pipeline (3b) > never a `--chat-template` flag at serve time.
  2. [medium] *Curl script isn't a CI regression barrier.* O12 adds a tracked `tests/e2e/online_serving/test_fun_audio_chat_chat_completions.py` covering non-streaming and streaming `/v1/chat/completions`, asserting `completion_tokens > 0`, non-null content, and matching wav output on both paths. Curl script kept around purely as a manual smoke harness. Also adds a server-side regression guard so any HTTP 200 with `completion_tokens == 0` is converted into an engine error (or at minimum a structured log that the CI test asserts on).
- 2026-04-22 v7 — `codex review --base fun-audio-chat-s2s-native-pipeline` (direct invocation; companion route kept hitting sandbox failures) surfaced two **concrete code-level bugs** on top of the plan:
  - [P1 / high] **`_encode_audio_prefill` uses padded Whisper feature width as audio length** (`fun_audio_chat.py:391-392`). `_call_hf_processor` pads features to 3000 frames (30 s). Using `feat.shape[-1]` as `feature_lens` treats every clip as 30 s, so the continuous encoder outputs ~150 fused embeddings for a 1 s clip while `_get_prompt_updates` inserted only 5 `<|AUDIO|>` placeholders. Masked-scatter mismatch → prefill embeddings are wrong for every real input, which is likely contributing to or compounding the 0-token symptom. Fix: thread `feature_attention_mask` through to `_encode_audio_prefill` and derive `feature_lens = attention_mask.sum(-1)`. Add to O12 diagnostic step 0.
  - [P2 / medium] **`token2wav` dummy-run guard is incomplete** (`token2wav.py:98-106`). vllm's `_dummy_run` passes a non-empty tensor of zero-valued token ids, which currently pass the `0 <= t < 6561` filter and trigger the full multi-GB CosyVoice load + vocoder run during worker warmup. Fix: short-circuit when ALL tokens are zero (dummy-run pattern) OR when the runner explicitly signals dummy mode; preserve real requests where tokens happen to include a valid 0.
  Both fixes land in tracked model code per the O12 non-negotiable rule.
- 2026-04-22 v8 — O12 landed (Stage 0 `final_output=True`, CRQState lazy-init, delta-emit in make_omni_output, `input_ids is None` guards, audio placeholder count, pipeline registry). Text path now works: `completion_tokens=512`, `message.content` is coherent English matching the standalone driver. CRQ produces 485 tokens through Stage-1 (observed: `FunAudioChatToken2Wav.forward: synthesised wav shape=(1, 446400) sr=24000` — 18.6 s of audio at 24 kHz). Remaining gap: `message.audio` is `null` in the HTTP response despite Stage-1 successfully vocoding. Added **O13** for the response-assembly bug (tracked-code-only per O12's non-negotiable rule).
