...
There is no separate prewarm command. Operators populate one matching producer
...
identities, while equivalent DP replicas share them. The BF16 contract covers
MiniMax H3 and `black-forest-labs/FLUX.2-klein-4B`; FLUX.2-klein evidence is
TP1-only, while its TP2/SP layouts rely on the existing per-coordinate identity
mechanics and remain unmeasured. The FP8 producer is limited to the eligibility
shown below. HSDP, LoRA/adapted weights, and non-default load formats remain
ineligible for FP8 HWR.
...
scales. Online INT8 and online MXFP8 linears use the ordinary loader before
DLO sharding. DLO reconstructs these tensors with their recorded physical
layouts: online INT8 has an int8 weight plus FP32 scale, while online MXFP8
has an FP8 weight plus an e8m0 block scale; both may use contiguous or
transposed views. Other online methods must select rank-local transfer or
