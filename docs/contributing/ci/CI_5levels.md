...
-   ***Performance Testing***: Uses `tests/dfx/perf/tests/test_qwen_omni.json` and `test_tts.json` (passed to `test_omni_benchmark.py` via `--config-file`) to drive performance testing tools for stress, load, and endurance tests, collecting metrics like throughput, response time, and resource utilization.
...
-   ***Performance Test Configuration***: `tests/dfx/perf/tests/test_qwen_omni.json`, `tests/dfx/perf/tests/test_tts.json`
