...
-   ***Performance Testing***: Uses `tests/dfx/perf/tests/test_qwen_omni.json` and `test_tts.json` (passed to `run_benchmark.py` via `--test-config-file`) to drive performance testing tools for stress, load, and endurance tests, collecting metrics like throughput, response time, and resource utilization.
