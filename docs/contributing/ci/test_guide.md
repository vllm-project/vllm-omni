...
=== "L3 level & L4 level"
    ...
    pytest -s -v -m "core_model and distributed_cuda and L4"  --run-level=core_model
    ```
    Note: To run performance tests (defaults to ``test_qwen_omni.json``; use ``--config-file tests/dfx/perf/tests/test_tts.json`` for TTS):
    ```bash
    pytest -s -v tests/dfx/perf/scripts/test_omni_benchmark.py
    ```
