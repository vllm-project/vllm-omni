...
Standalone stages communicate via `/v1/stage/run`. A request without
`stage_output` runs the model and returns raw multimodal output. A request with
`stage_output` accepts upstream output and returns the final result. An external
system chains stages by calling the first stage and forwarding its response to
the next. See `docs/user_guide/standalone_disaggregation.md` for details.
