...
OpenAI-compatible API server. This page covers the text and audio surface;
diffusion / image / video metrics are tracked in a follow-up PR.
...
server-side observation aligns with the bench-side definition.
...
- All time-bearing metrics use the `_s` suffix (values in seconds).
  ...
  `SECONDS_FAST_BUCKETS` (1 ms → 60 s) for the fine-grained transfer and
  audio-underrun values.
...
- Text and audio first-output use distinct families
  (`vllm:time_to_first_token_seconds` reused from upstream for text;
