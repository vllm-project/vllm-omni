...
- 每层包含 camera condition injector，输入是 checkpoint 所称的 6-channel camera ray embedding；
...
└── models/wan2_2/
    ...
    ├── lingbot_world_camera.py         # poses/intrinsics → camera ray embedding
...
- `lingbot_world_camera.py` 只负责 numpy/tensor camera 数据校验、插值、相对位姿和 camera ray embedding；
...
3. 先在原始 camera-to-world poses 上插值到 latent 帧数，再转为相对第一帧的 poses，随后计算逐帧 delta（第 0 帧为 identity，第 `i` 帧为 `inv(relative[i-1]) @ relative[i]`），并以全序列最大 translation norm 归一化平移。像素坐标使用 half-pixel center；六个通道严格为 `[ray_origin_xyz, normalized_ray_direction_xyz]`，再按 8x8 spatial fold 重排为 `[B, 6*8*8, F_latent, H_latent, W_latent]`。
4. 将首帧 resize 到目标分辨率，与黑帧拼接后 VAE encode；按 `[mask4, image_latent16]` 构造 20-channel condition，因此 transformer 的 36-channel 输入严格为 `[noise16, mask4, image_latent16]`。
...
1. camera pose/intrinsics shape 校验、raw-load、插值后 framewise delta、translation 归一化、half-pixel ray、channel order 和 fold 输出 shape；
2. latent geometry、`[noise16, mask4, image_latent16]` 精确 channel 位置和 chunk 对齐；
...
因此只提交测试入口，不宣称本地 GPU 通过。真实 checkpoint 的 golden-video 质量对比明确延后到
具备官方权重和 GPU 的验证环境；本地 CPU/stub 测试不冒充视觉 golden。
...
Apache-2.0 的 SGLang 适配实现。上述 camera contract 是按 Apache SGLang checkpoint adapter
校正后的契约，取代早期设计中的“load 时先相对化”和 `(direction, origin × direction)` 约定；不直接复制 `robbyant/lingbot-world-v2` 的
