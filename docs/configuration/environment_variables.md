...
| Name | Type and default | Applies to and read time | Precedence and invalid values | Lifecycle |
| --- | --- | --- | --- | --- |
| `VLLM_OMNI_FIXED_KV_DECODE` | `0` disables; default `1` (on) | Fixed-capacity decode attention backend and its ACL-graph hook; read when the NPU platform resolves the attention backend | Only the exact value `0` disables it. Scoped by `max_model_len`: a long-context stage (e.g. a 32768-token Thinker) declines and keeps the stock path. | Experimental performance control |
| `VLLM_OMNI_FIXED_KV_DECODE_BUCKETS` | Comma-separated KV capacities in tokens; default `512,4096` | Capture buckets of the fixed-KV decode graphs; read when the buckets are computed | The full capacity is always kept as the top bucket, so a longer sequence still has a graph. Empty or malformed entries are skipped. | Experimental performance control |
| `VLLM_OMNI_FIXED_KV_DECODE_MAX_LEN` | Integer tokens; default `8192` | Fixed-KV decode eligibility; read per step when attention decides whether to engage | At or below this length the fixed-KV path is used; above it the stock path runs. | Experimental performance control |
| `VLLM_OMNI_NPU_SKIP_WARMUPS` | Comma-separated warmup-name substrings, or `none`; default `penalties,rejection_sampler` on 910_93 and `rejection_sampler` on 910B | vllm-ascend Triton kernel warmups on the NPU worker; resolved per warmup call | Skips the warmups whose dummy-token setup faults the vector core (acl 507035). `none` restores the stock warmups. | Experimental |
