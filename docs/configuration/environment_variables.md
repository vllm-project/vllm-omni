...
| `VLLM_OMNI_AR_DIFFUSION_ALLOW_REFERENCE_ATTN` | `1`, `true`, `yes` or `on` enables; default `0` (off) | AR-Diffusion paged self-attention; read per attention call | Environment-only setting. Values are stripped and case-normalized. **Diagnostic only.** By default an accelerator whose paged FlashAttention kernel is unavailable raises; enabling this substitutes the dense Python reference instead, which is far slower. Do not set it in a deployment. | Diagnostic |
