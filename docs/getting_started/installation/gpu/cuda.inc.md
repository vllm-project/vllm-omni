...
The release {{ vllm_version }} of vLLM is based on PyTorch 2.9.0 which requires CUDA 12.9 environment.
...
```bash
export VLLM_PRECOMPILED_WHEEL_LOCATION=https://github.com/vllm-project/vllm/releases/download/v{{ vllm_version }}/vllm-{{ vllm_version }}-cp38-abi3-manylinux_2_31_x86_64.whl
```
...
docker run --runtime nvidia --gpus 2 \
    ...
    -p 8091:8091 \
    --ipc=host \
    vllm/vllm-omni:v{{ vllm_omni_version }} \
