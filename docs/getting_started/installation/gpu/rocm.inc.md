...
```bash
uv pip install vllm=={{ vllm_version }}+rocm700 --extra-index-url https://wheels.vllm.ai/rocm/{{ vllm_version }}/rocm700
```
...
The release {{ vllm_version }} of vLLM requires ROCm 7.0 environment.
...
docker run --rm \
  ...
  --env "HF_TOKEN=$HF_TOKEN" \
  -p 8091:8091 \
  vllm/vllm-omni-rocm:v{{ vllm_omni_version }} \
