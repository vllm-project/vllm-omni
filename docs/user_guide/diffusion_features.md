...
| **LongCat-Image**        |     ✅     |     ✅      |           ✅           |       ✅        |         ✅         |          ❌          |   ❌    |             ✅             |          ❌           |       ❌        |        ❌         |
| **LongCat-Image-Edit**   |     ✅     |     ✅      |           ✅           |       ✅        |         ✅         |          ❌          |   ❌    |             ✅             |          ❌           |       ❌        |        ❌         |
| **Mage-Flow (T2I & Edit)** | ❌   |     ❌      |      ✅ (Ulysses)      |   ✅ (2 only)   |         ✅         |          ❌          |   ❌    |             ❓             |          ❌           |       ❌        |        ❌         |
| **MammothModa2(T2I)**    |     ❌     |     ❌      |           ❌           |       ❌        |         ❌         |          ❌          |   ❌    |             ❌             |          ❌           |       ❌        |        ❌         |
| **Nextstep_1(T2I)**      |     ❓     |     ❓      |           ❌           |       ✅        |         ✅         |          ❌          |   ❌    |             ✅             |          ❌           |       ❌        |        ❌         |
| **OmniGen2**             |     ❌     |     ✅      |           ✅           |       ❌        |         ✅         |          ❌          |   ❌    |             ❌             |          ❌           |       ❌        |        ❌         |
| **Ovis-Image**           |     ❌     |     ✅      |           ❌           |       ✅        |         ❌         |          ❌          |   ❌    |             ✅             |          ❌           |       ❌        |        ❌         |
...
> 7. Mage-Flow shares one pipeline for text-to-image and editing, so both have identical feature support. Only Ulysses sequence parallelism is exercised; ring attention is untested. SP requires uniform token counts across the batch, so it is refused at startup unless `--max-num-seqs 1`; under SP a guided request runs its two guidance branches as sequential forwards instead of one packed forward, and pairing with `--cfg-parallel-size 2` is faster because it splits them by rank. CFG-Parallel splits exactly one positive and one negative branch, so `--cfg-parallel-size` must be 1 or 2. Layerwise offload is wired on `transformer_blocks` but is not validated by any test or measurement, hence ❓. The upstream reference implementation additionally runs a mandatory content-policy gate and embeds a Gaussian-Shading provenance watermark; this integration provides neither — see the recipe's Known limitations.
