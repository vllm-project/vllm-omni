...
> 7. MammothModa2 runs its DiT stage on the generation runner, so Cache-DiT is configured on the DiT stage entry of the deploy YAML (`cache_backend: cache_dit` on stage 1) rather than through the diffusion-stage knobs. Only the repeated main-layer stack is cached; requests with `text_guidance_scale = 1.0` bypass the cache hooks.
