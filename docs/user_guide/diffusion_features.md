...
> 8. SenseNova-U1.5 step execution covers t2i (with and without think), it2i, and text output (text finishes inside the prepare phase), including step-wise batching with `--max-num-seqs > 1`; each wave runs one transformer forward per request (no packed forward), and the model-local decode cache serializes the think/text phases. It cannot be combined with a diffusion cache backend; see [Diffusion Execution Modes](diffusion/execution_modes.md#step-execution).
...
> use `max_num_seqs=1`. SenseNova-U1 / U1.5 batch their step execution (see
> image-model note 8).
