...
> 3. **DeepSeek Janus** ([`JanusPipeline`](../models/supported_models.md)) uses autoregressive image-token prediction plus VQ decode rather than a classical DiT denoising loop. TeaCache/Cache-DiT, CFG parallelism, tensor parallelism, VAE patch parallelism, and step execution are not wired for this single-stage implementation. Layerwise/module-wise CPU offload and quantization are applicable. Sequence parallelism may be possible through the AR transformer path but is not validated in vLLM-Omni today. The public deployment keeps Janus prompt formatting, CFG pairing, AR token generation, and VQ decode inside one single-stage pipeline.
> 4. Cosmos3 T2I uses `Cosmos3OmniDiffusersPipeline` with `modalities=["image"]`. Model-level CPU offload swaps the nested UND reasoner and GEN generator pathways; layerwise offload remains available for blockwise GEN/UND offload.
