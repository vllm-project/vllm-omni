...
When requesting audio output (`modalities: ["audio"]`), you can control the TTS voice and language in the request.

**Speaker**

Pass a `speaker` field in the request body; it is stored in `additional_information` and used by the talker stage. Supported values depend on the Qwen2.5-Omni checkpoint; typical built-in voices include:
...
Pass a `language` field for explicit language control when the pipeline supports it.
