...
class OmniOpenAIServingChatBatch(OmniOpenAIServingChat):
    ...
    async def render_batch_chat_request(
        ...
        """
        # --- Batch-level checks (run once for the whole batch) ---
        ...
        # Check 7: Chat template validation
        ...
        # Check 8: Output modalities validation
        ...
        # Check 9: Audio format validation
        ...
        # --- Per-item checks and preprocessing ---
        ...
        for messages in request.messages:
            # Per-item 1: Convert batch entry to a single ChatCompletionRequest
            ...
            # Per-item 2: Mistral tokenizer normalization
            if isinstance(tokenizer, MistralTokenizer):
                maybe_serialize_tool_calls(single_request)
                truncate_tool_call_ids(single_request)
                validate_request_params(single_request)

            # Per-item 3: Chat preprocessing (template rendering, prompt tokenization)
            try:
                if not self.use_harmony:
    ...
    async def chat_completion_full_generator_batch(
        ...
        for prompt_idx in range(len(generators)):
            ...
            if single_requests is not None:
    ...
    async def create_batch_chat_completion(
        ...
        response.  Falls back to the legacy path for diffusion models.
        """
        if is_single_stage_diffusion(self.engine_client):
