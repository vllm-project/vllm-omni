...
class OmniOpenAIServingSpeech(OpenAIServing, AudioMixin):
    ...
    async def _build_voxtral_prompt(self, request: OpenAICreateSpeechRequest) -> dict[str, Any]:
        ...
        if voice is not None:
            # Check if it's an uploaded voice with stored reference audio
            ...
            if voice_lower in self.uploaded_speakers:
                ...
                # Get reference audio from stored file
                ref_audio_data = self._get_uploaded_audio_data(voice)
                if ref_audio_data is None:
                    raise ValueError(
                        f"Reference audio for uploaded voice '{voice}' is missing or unreadable. "
                        "Please re-upload the voice sample and try again."
                    )
                # Strip data URI prefix
                _, _, ref_audio = ref_audio_data.partition(",")
            else:
                # Built-in voice name — pass directly to tokenizer
        ...
        # Use ref_audio (either from request or resolved from uploaded voice)
