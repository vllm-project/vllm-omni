        ...
        if isinstance(devices, str) and "," in devices:
            ...
            if toks:
                try:
                    ...
                    logger.debug(
                        "[Stage-%s] Set CUDA_VISIBLE_DEVICES to %s; logical 0 -> physical %s",  # noqa: E501
                        stage_id,
                        devices,
                        selected_physical,
                    )
            ...
            if vis:
                try:
                    ...
                    logger.debug(
                        "[Stage-%s] Failed to map logical index via CUDA_VISIBLE_DEVICES: %s",  # noqa: E501
                        stage_id,
                        e,
                    )
                    selected_physical = None
            ...
            logger.debug(
                "[Stage-%s] Logical index %d -> physical %s; set CUDA_VISIBLE_DEVICES to single device",  # noqa: E501
                stage_id,
                logical_idx + 1,
                selected_physical,
            )
...
def shm_write_bytes(payload: bytes) -> Dict[str, Any]:
    ...
    try:
        ...
        logger.debug("Ensure parent dir failed for %s", e)
    return meta
