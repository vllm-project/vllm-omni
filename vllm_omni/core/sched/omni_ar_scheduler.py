...
class OmniARScheduler(OmniSchedulerMixin, VLLMScheduler):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self._hbm_admission_enabled = (
            getattr(self.vllm_config.model_config, "hbm_limit_gb", None)
            is not None
    ...
    def _should_defer_waiting_admission(self) -> bool:
        ...
        if should_defer:
            self.hbm_admission_deferred_steps = (
                getattr(self, "hbm_admission_deferred_steps", 0) + 1
            )
            waiting_count = len(self.waiting)
            self.hbm_admission_deferred_requests = (
                getattr(self, "hbm_admission_deferred_requests", 0)
                + waiting_count
            )
            if not was_active:
                logger.info(
                    "[HBMAdmission] paused free_blocks=%d total_blocks=%s "
                    "waiting=%d running=%d deferred_steps=%d",
                    free_blocks,
            ...
            self.hbm_admission_resume_count = (
                getattr(self, "hbm_admission_resume_count", 0) + 1
            )
            logger.info(
