...
This module provides shared logic for AR schedulers, including:
- update_from_output with check_stop bug fix for multimodal models
- _enrich_scheduler_output for Omni-specific payloads

OmniGenerationScheduler should NOT inherit from this class as it has
fundamentally different scheduling semantics (one-step generation for DiT).
"""
...
class BaseOmniARScheduler(BaseOmniScheduler):
    ...
    def update_from_output(
        ...
        is empty. This causes IndexError during prefill phase when pooler_outputs
        exist but output_token_ids is still empty.
