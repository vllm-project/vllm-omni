...
def create_request_queue(
    policy: OmniSchedulingPolicy,
    all_modalities_mask: Optional[int] = 0
) -> RequestQueue:
...
class ModalityAwareRequestQueue(RequestQueue):
    ...
    def bind_modality_config(self, all_modalities_mask: int) -> None:
        ...
        assert (all_modalities_mask > 0 and 
                (all_modalities_mask & (all_modalities_mask + 1)) == 0)
    ...
    def pop_request(self) -> Request:
        ...
        for mask, bucket in self._modality_buckets.items():
            ...
            if (min_req is None or 
                    current_head.arrival_time_mono < min_req.arrival_time_mono):
    ...
    def peek_request_by_mm_mask(
        self,
        mm_mask: int,
        filter_policy: MaskFilterPolicy = MaskFilterPolicy.EXACT
    ) -> Request:
        ...
        if mm_mask not in self._modality_buckets:
            raise KeyError(
                "this modality combination is not supported by the model"
            )
    ...
    def prepend_requests(self, requests: RequestQueue) -> None:
        ...
        if not isinstance(requests, ModalityAwareRequestQueue):
            raise TypeError(
                "Omni-Scheduler requires ModalityAwareRequestQueue for "
                "consistency."
            )
        
        for mask, bucket in self._modality_buckets.items():
            if (mask not in requests._modality_buckets or 
                    mask not in requests._modality_stats):
                raise KeyError(
                    "the two ModalityAwareRequestQueue should have the same "
                    "bucket masks"
                )
    ...
    def remove_request(self, request: Request) -> None:
        ...
        if mask not in self._modality_buckets:
            raise ValueError(
                "this request cannot be in this queue as its modality "
                "is not supported by the model config"
            )
        
        if not self._modality_buckets[mask]:
            raise ValueError(
                "this request cannot be in this queue as its bucket is empty"
            )
    ...
    def __iter__(self) -> Iterator[Request]:
        ...
        """
        active_bucket_iters = [
            iter(bucket) for bucket in self._modality_buckets.values() if bucket
        ]
        yield from heapq.merge(
            *active_bucket_iters,
            key=lambda r: r.arrival_time_mono
        )
    ...
    def __reversed__(self) -> Iterator[Request]:
        ...
        """
        active_bucket_reversed_iters = [
            reversed(bucket) 
            for bucket in self._modality_buckets.values() if bucket
        ]
        yield from heapq.merge(
            *active_bucket_reversed_iters,
            key=lambda r: r.arrival_time_mono,
    ...
    def get_reversed_bucket(self, mm_mask: int) -> Deque[Request]:
        ...
        if mm_mask not in self._modality_buckets:
            raise KeyError(
                "cannot reverse a bucket when its mask is not in the queue"
            )
    ...
    def get_compatible_mm_submask(self, current_resource_mask: int) -> set[int]:
        ...
        while submask > 0:
            if (submask in self._modality_buckets and 
                    self._modality_buckets[submask]):
