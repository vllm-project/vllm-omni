...
class AsyncOmniOutputRunnerMixin:
        ...
        *,
        ...
        if needs_scheduled_hidden_payload:
            if staged_hidden_states_cpu is None:
                # NPU does not pre-stage hidden states. Prefix-cache output is
                # synchronous, so copy only the scheduled rows when needed.
                num_valid_tokens = min(
                    int(scheduler_output.total_num_scheduled_tokens),
                    int(hidden_states.shape[0]),
                )
                staged_hidden_states_cpu = _to_cpu_contiguous(hidden_states[:num_valid_tokens])
    ...
    def _build_omni_step_outputs(
        ...
        *,
    ...
    ) -> tuple[list[dict[str, Any] | None] | None, list[dict[str, object]] | None]:
        ...
        # Inter-stage slots may be None (empty half after partition); the client
        # channel keeps the msgspec tensor-only list[dict] shape.
        return cast(list[dict[str, Any] | None] | None, inter_stage_outputs), multimodal_outputs
