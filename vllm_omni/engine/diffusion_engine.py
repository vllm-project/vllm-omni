    ...
    def __init__(
        ...
        if model_cfg is not None:
            if isinstance(model_cfg, dict):
                config_model_path = model_cfg.get("model")
            else:
                config_model_path = getattr(model_cfg, "model", None)
...
class DiffusersPipelineEngine(DiffusionEngine):
    ...
    def __init__(
        ...
        resolved_model_path = self.model_path or None
        ...
        if device_cfg:
            if isinstance(device_cfg, dict):
                ...
                dtype = device_cfg.get("dtype", None)
            else:
