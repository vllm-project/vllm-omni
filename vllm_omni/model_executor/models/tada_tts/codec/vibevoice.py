"""Vendored TADA diffusion head (VibeVoice).

Adapted verbatim from HumeAI's ``hume-tada`` package (``tada/nn/vibevoice.py``),
which is MIT-licensed. Vendored here so the vLLM-Omni TADA integration does not
take a runtime dependency on ``hume-tada``; only the codec *weights* are still
fetched from ``HumeAI/tada-codec``. Class and parameter names are kept identical
so pretrained checkpoints load without remapping.
...
class VibeVoiceDiffusionHead(PreTrainedModel):
    ...
    def __init__(
        ...
        # Required by newer transformers for from_pretrained bookkeeping.
        self.post_init()
