        ...
        **kwargs: object,
        ...
        if self.model_stage=="thinker":
            ...
            if sampler is not None:
                output_hidden = text_hidden_states.squeeze(0) if added_batch_dim else text_hidden_states
                sample_hidden = output_hidden[logits_index]
                logits = self.compute_logits(sample_hidden, None)
                sampler_output = sampler(logits, sampling_metadata)
                sampler_output_ids = sampler_output.sampled_token_ids
                if self.thinker_output_token_ids.shape[0]==0:
                    self.prev_inputs = input_ids
                self.thinker_output_token_ids = torch.cat([self.thinker_output_token_ids, sampler_output_ids], dim=1)
                self.thinker_hidden_states = torch.cat([self.thinker_hidden_states, output_hidden], dim=0)
                
            else:
                sampler_output_ids = None
            ...
            if sampler is None:
                return OmniOutput(
                    text_hidden_states=torch.zeros([inputs_embeds.shape[0],896], dtype=torch.bfloat16).cuda(),
                    multimodal_outputs=None
                )
        ...
        if self.model_stage=="talker":
            ...
            if self.is_prefill:
                # talker_inputs_embeds_len = self.last_index_of(input_ids.cpu().tolist(), 8293) + 1
                talker_inputs_embeds = inputs_embeds#[:talker_inputs_embeds_len]
                thinker_result = additional_hidden_states#torch.load(os.environ["thinker_hidden_states_dir"]+f"/{hash_tensor_bytes(inputs_embeds)}.pt", weights_only=False)#inputs_embeds[talker_inputs_embeds_len:]
                # os.remove(os.environ["thinker_hidden_states_dir"]+f"/{hash_tensor_bytes(inputs_embeds)}.pt")
                ...
                talker_inputs_ids, talker_inputs_embeds = input_ids, talker_inputs_embeds #input_ids[:talker_inputs_embeds_len], talker_inputs_embeds
                # self._thinker_to_talker_prefill(
                #     voice_type=voice_type,
                #     output_prompt_embeds=thinker_result.squeeze(0),
                #     output_token_ids=self.thinker_output_token_ids.squeeze(0).cpu().tolist(),
                #     thinker_prompt_embeds=self.thinker_hidden_states[:self.prev_inputs.shape[1]], #self.thinker.get_input_embeddings(self.prev_inputs.squeeze(0))+self.thinker_hidden_states[:self.prev_inputs.shape[1]],
                #     prompt_token_ids=self.prev_inputs.squeeze(0).cpu().tolist(),
                # )
                # talker_positions = torch.arange(start=0, end=talker_inputs_ids.shape[0],dtype=torch.int).cuda()
                sampling_metadata.prompt_token_ids = talker_inputs_ids.unsqueeze(0)
                # empty_makeup_tensor = torch.zeros([positions.shape[1] - talker_inputs_embeds_len, self.talker_config.hidden_size], dtype=torch.bfloat16).to(talker_inputs_embeds.device)
                # positions = positions[:, :talker_inputs_embeds_len]
                # sampling_metadata.output_token_ids = torch.empty((1,0))
            else:
                talker_inputs_ids, talker_inputs_embeds = \
            ...
            # if self.is_prefill:
            #     talker_hidden = torch.cat([empty_makeup_tensor, talker_hidden])

            self.is_prefill = False
            # print(sampling_metadata.output_token_ids)
            # torch.save(sampling_metadata.output_token_ids, os.environ["talker_output_path"])
        ...
        if self.model_stage=="code2wav":
            code = input_ids #torch.load(os.environ["talker_output_path"], weights_only=False)
