...
def moe_forward(
    ...
    # Guard against silent corruption from an under-sized mask. The message
    # closure must stay constant-only; tensor-capturing closures graph-break
    # under fullgraph=True.
    torch._check(
        flat_mask.shape[0] == flat_hid.shape[0],
        lambda: "gen_token_mask must have exactly one entry per hidden_states token",
    )
...
class MammothModa2Qwen2ForCausalLM(nn.Module, SupportsPP):
    ...
    def get_input_embeddings(self, input_ids: torch.Tensor) -> torch.Tensor:
        # `async_scheduling=True` can feed a `-1` sentinel for slots whose
        # sampled token has not been written back yet; the AR deploy configs
        # pin `async_scheduling: false` for that reason (#7319). Clamp
        # negatives so both gathers below (and the short-circuit branch) stay
