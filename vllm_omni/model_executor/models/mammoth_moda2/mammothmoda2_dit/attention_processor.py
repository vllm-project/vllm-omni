...
# transformers 的 flash-attn utils 在不同版本里符号不完全一致（例如缺少 flash_attn_varlen_func）。
# 这里做兼容导入：即使 flash-attn 不可用，也允许模块成功 import，从而回退到普通 Attention processor。
try:
    from transformers.modeling_flash_attention_utils import (  # type: ignore
        ...
        pad_input,  # pyright: ignore[reportAttributeAccessIssue]
        unpad_input,  # pyright: ignore[reportAttributeAccessIssue]
    )
