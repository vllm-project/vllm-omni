...
def generate_audio(
    ...
    for i in range(max_generation_len):
        ...
        curr_tokens_mask = torch.cat(
            [torch.ones_like(frame).bool(), torch.zeros(1, 1).bool().to(device)], dim=1
        ).unsqueeze(1)
...
def main():
    ...
    parser.add_argument(
        "--max_audio_length_ms", type=float, default=5000, help="Max audio length in ms (default: 5000)"
    )
    ...
    parser.add_argument(
        "--model_path", type=str, default=None, help="Model path or HF repo ID (default: MisoLabs/MisoTTS)"
    )
