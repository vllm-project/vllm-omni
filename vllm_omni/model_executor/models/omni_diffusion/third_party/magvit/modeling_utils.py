...
class ModelMixin(torch.nn.Module, PushToHubMixin):
    ...
    def from_pretrained(cls, pretrained_model_name_or_path: str | os.PathLike | None, **kwargs):
        ...
        if from_flax:
            raise ValueError("Omni-Diffusion MagVIT does not support loading Flax checkpoints.")
        ...
        model_file = None
        if from_flax:
            raise AssertionError("Flax loading is rejected before model resolution.")
        else:
            if is_sharded:
            ...
            if low_cpu_mem_usage:
                ...
                def load_model_in_low_cpu_mem_usage(
                    config, device_map, unused_kwargs, commit_hash, is_sharded, index_file
                ):
                    ...
                    if device_map is None and not is_sharded:
                        ...
                        load_into_meta_kwargs = {
                            "dtype": torch_dtype,
                            "model_name_or_path": pretrained_model_name_or_path,
                        }
                        load_into_meta_parameters = inspect.signature(load_model_dict_into_meta).parameters
                        if "device" in load_into_meta_parameters:
                            load_into_meta_kwargs["device"] = param_device
                        elif "device_map" in load_into_meta_parameters:
                            load_into_meta_kwargs["device_map"] = {"": param_device}
                        load_model_dict_into_meta(model, state_dict, **load_into_meta_kwargs)
                        unexpected_keys = set(state_dict) - set(model.state_dict())
                ...
                model, loading_info = load_model_in_low_cpu_mem_usage(
                    config, device_map, unused_kwargs, commit_hash, is_sharded, index_file
                )
