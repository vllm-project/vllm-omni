...
def _falcon_config_view(vllm_config: VllmConfig) -> VllmConfig:
    """Return a shallow ``vllm_config`` copy whose ``model_config.hf_config`` is
    the Falcon-H1 text_config.

    vLLM's ``FalconH1Model`` and ``FalconH1ForCausalLM``'s mamba-state
    classmethods read ``vllm_config.model_config.hf_config`` directly (unlike
    Qwen2Model, which calls ``get_text_config()``), so they must see the
    Falcon-H1 sub-config -- carrying ``vocab_size``, ``mamba_*`` and the muP
    multipliers -- rather than the arktts wrapper that only exposes those on
    ``text_config``.
    """
    text_config = vllm_config.model_config.hf_config.get_text_config()
    falcon_vllm_config = copy.copy(vllm_config)
    falcon_model_config = copy.copy(vllm_config.model_config)
    falcon_model_config.hf_config = text_config
    falcon_model_config.hf_text_config = text_config
    falcon_vllm_config.model_config = falcon_model_config
    return falcon_vllm_config


class Audio8TTS01BSlowARForConditionalGeneration(Audio8TTSSlowARForConditionalGeneration, HasInnerState, IsHybrid):
    ...
    vLLM sizes the Mamba2 state cache for the Falcon-H1 backbone; the state
    shape/dtype/copy classmethods mirror ``FalconH1ForCausalLM`` but read the
    Falcon-H1 sub-config (``get_text_config()``) instead of the arktts wrapper.
    """

    @classmethod
    def get_mamba_state_dtype_from_config(cls, vllm_config: VllmConfig):
        return FalconH1ForCausalLM.get_mamba_state_dtype_from_config(_falcon_config_view(vllm_config))

    @classmethod
    def get_mamba_state_shape_from_config(cls, vllm_config: VllmConfig):
        return FalconH1ForCausalLM.get_mamba_state_shape_from_config(_falcon_config_view(vllm_config))
    ...
    def __init__(self, *, vllm_config: VllmConfig, prefix: str = ""):
        ...
        #
        # Unlike Qwen2Model (which calls hf_config.get_text_config()), vLLM's
        # FalconH1Model reads vllm_config.model_config.hf_config directly, so it
        # must see the Falcon-H1 text_config (vocab_size, mamba_*, muP) rather
        # than the arktts wrapper. Hand it a config view with hf_config swapped.
        falcon_vllm_config = _falcon_config_view(vllm_config)
        with set_current_vllm_config(falcon_vllm_config):
            self.model = FalconH1Model(vllm_config=falcon_vllm_config, prefix=maybe_prefix(prefix, "model"))
        ...
        # Fast AR: separate compilation context, reused verbatim from 0.6b.
