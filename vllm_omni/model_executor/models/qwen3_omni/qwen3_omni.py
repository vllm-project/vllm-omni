...
class Qwen3OmniMoeForConditionalGeneration(
    ...
    def talker_preprocess_decode(self, input_ids: torch.Tensor, input_embeds: torch.Tensor, **info_dict: dict):
        ...
        if isinstance(q_tail, torch.Tensor) and q_tail.numel() > 0:
            ...
            new_q_tail = (
                ...
                if q_tail.shape[0] > 1 or (self.async_chunk_stream and not thinker_eos)
    ...
    def _get_talker_assistant_parts_for_decode(self, thinker_embed, tts_eos_embed, thinker_end_token=False):
        ...
        if assistant_hidden.ndim != tts_eos_embed.ndim:
            if assistant_hidden.ndim == tts_eos_embed.ndim - 1:
                tts_eos_embed = tts_eos_embed.squeeze(0)
            else:
                raise ValueError(
                    f"Dimension mismatch: assistant_hidden.ndim={assistant_hidden.ndim}, "
                    f"tts_eos_embed.ndim={tts_eos_embed.ndim}"
                )
        ...
        if thinker_end_token:
            trailing_text_hidden = torch.cat(
                (
                    assistant_hidden,
                    tts_eos_embed.to(assistant_hidden.device, dtype=assistant_hidden.dtype),
                ),
                dim=0,
            )
        else:
