...
# NOTE: they declared these global vars
mel_basis = {}
hann_window = {}


def dynamic_range_compression_torch(x, C=1, clip_val=1e-5):  # noqa: N803
...
def mel_spectrogram(
    ...
    if f"{str(fmax)}_{str(y.device)}" not in mel_basis:
        # Shared helper returns (n_mels, n_freqs) matching librosa's layout,
        # so the downstream matmul stays identical.
