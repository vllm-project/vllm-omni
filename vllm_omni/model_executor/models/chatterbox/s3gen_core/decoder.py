...
class ConditionalDecoder(nn.Module):
    def __init__(
        ...
        for i in range(len(channels)):  # pylint: disable=consider-using-enumerate
            ...
            _cls = CausalResnetBlock1D if self.causal else ResnetBlock1D
            resnet = _cls(
                dim=input_channel,
            ...
            if not is_last:
                ...
                downsample = CausalConv1d(output_channel, output_channel, 3)
            else:
        ...
        for i in range(len(channels) - 1):
            ...
            if not is_last:
                ...
                upsample = CausalConv1d(output_channel, output_channel, 3)
            else:
                upsample = nn.Conv1d(output_channel, output_channel, 3, padding=1)
            self.up_blocks.append(nn.ModuleList([resnet, transformer_blocks, upsample]))
        _final_cls = CausalBlock1D if self.causal else Block1D
        self.final_block = _final_cls(channels[-1], channels[-1])
