...
# S3Tokenizer is used only in embed_ref() (cold path, cached per speaker),
# so we keep it as an upstream import rather than vendoring its sources.
...
def drop_invalid_tokens(x):
    # vllm-omni vendor: upstream asserted B==1. We drop the assert because
    # the vllm-omni caller (ChatterboxS3Gen._filter_tokens) already filters
    # per-request before batching, and this function is no longer on the
    # hot path. Kept for backwards compatibility with B==1 callers only.
    if x.dim() == 1:
        return x[x < SPEECH_VOCAB_SIZE]
    # Per-row filter for B>1 — returns a list since rows may differ in length.
...
class S3Token2Wav(S3Token2Mel):
    ...
    def flow_inference(
        ...
        noise = None
        if self.meanflow:
            # vllm-omni vendor: use speech_tokens.size(0) for the batch dim
            # so meanflow noise broadcasts correctly when B > 1.
            noise = torch.randn(
    ...
    def hift_inference(self, speech_feat, cache_source: torch.Tensor = None):
        if cache_source is None:
            # vllm-omni vendor: size-0 cache per batch entry; upstream used
            # (1, 1, 0) which silently dropped cache rows for B > 1.
