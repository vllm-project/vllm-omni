...
class Qwen2_5OmniConditionalGenerationMixin(Qwen2_5OmniConditionalGenerationMixinBase):
    def _parse_and_validate_audio_input(
        ...
        if (
            ...
            input_audio_features = input_audio_features.reshape(
                -1, input_audio_features.shape[-1]
            )
        elif input_audio_features is not None and isinstance(
            input_audio_features, list
        ):
        ...
        elif audio_feature_lengths is not None and isinstance(
            audio_feature_lengths, list
        ):
        ...
        if (
            ...
            feature_attention_mask = feature_attention_mask.reshape(
                -1, feature_attention_mask.shape[-1]
            )
        elif feature_attention_mask is not None and isinstance(
            feature_attention_mask, list
        ):
    ...
    def _parse_and_validate_image_input(
        ...
        **kwargs: dict[str, Any],
        ...
        if (
            ...
            and pixel_values.ndim == 3
        ):
            pixel_values = pixel_values.reshape(-1, pixel_values.shape[-1])
        if (
            image_embeds is not None
            and isinstance(image_embeds, torch.Tensor)
            and image_embeds.ndim == 3
        ):
        ...
        **kwargs: dict[str, Any],
        ...
        if (
            ...
            pixel_values_videos = pixel_values_videos.reshape(
                -1, pixel_values_videos.shape[-1]
            )
        if (
            video_grid_thw is not None
            and isinstance(video_grid_thw, torch.Tensor)
            and video_grid_thw.ndim == 3
        ):
            video_grid_thw = video_grid_thw.reshape(-1, video_grid_thw.shape[-1])
        if (
            video_embeds is not None
            and isinstance(video_embeds, torch.Tensor)
            and video_embeds.ndim == 3
        ):
