...
def _speech_collate_fn(batch, pad_id, las_format):
    ...
    if las_format:
        # exclude EOS
        src_tokens = tokens[:, :-1]
        src_tokens_len = tokens_lengths - 1

        tgt_tokens = torch.zeros(tokens.shape[0], tokens.shape[1] - 1, dtype=tokens.dtype)
        # shift by 1
        tgt_tokens.copy_(tokens[:, 1:])
        # exclude SOS
        tgt_tokens_len = tokens_lengths - 1
        return audio_signal, audio_lengths, src_tokens, src_tokens_len, tgt_tokens, tgt_tokens_len
    else:
        return audio_signal, audio_lengths, tokens, tokens_lengths


def _speech_aux_collate_fn(batch, pad_id):
    """collate batch of audio sig, audio len, tokens, tokens len
    Args:
        batch (Optional[FloatTensor], Optional[LongTensor], LongTensor,
               LongTensor):  A tuple of tuples of signal, signal lengths,
               ...
               assumes the signals are 1d torch tensors (i.e. mono audio).
    """
    _, audio_lengths, _, tokens_lengths, _, aux_tokens_lengths = zip(*batch)
    max_audio_len = 0
    has_audio = audio_lengths[0] is not None
    if has_audio:
        max_audio_len = max(audio_lengths).item()
    ...
    max_aux_tokens_len = max(aux_tokens_lengths).item()

    audio_signal, tokens, aux_tokens = [], [], []
    for sig, sig_len, tokens_i, tokens_i_len, aux_tokens_i, aux_tokens_i_len in batch:
        if has_audio:
            sig_len = sig_len.item()
            if sig_len < max_audio_len:
                pad = (0, max_audio_len - sig_len)
        ...
        aux_tokens_i_len = aux_tokens_i_len.item()
        if aux_tokens_i_len < max_aux_tokens_len:
            pad = (0, max_aux_tokens_len - aux_tokens_i_len)
            aux_tokens_i = torch.nn.functional.pad(aux_tokens_i, pad, value=pad_id)
        aux_tokens.append(aux_tokens_i)
    ...
    aux_tokens = torch.stack(aux_tokens)
    aux_tokens_lengths = torch.stack(aux_tokens_lengths)

    return audio_signal, audio_lengths, tokens, tokens_lengths, aux_tokens, aux_tokens_lengths
...
class _AudioTextDataset(Dataset):
    ...
    def __init__(
        ...
        self.parser = parser
        self._add_aux_tokens = aux_parser is not None
        ...
        if self.las_format:
            assert not self._add_aux_tokens
    ...
    def __getitem__(self, index):
        ...
        if self._add_aux_tokens:
            aux_tokens = sample.aux_tokens
            aux_tokens_num = len(aux_tokens)
            output += (torch.tensor(aux_tokens).long(), torch.tensor(aux_tokens_num).long())
    ...
    def _collate_fn(self, batch):
        if self._add_aux_tokens:
            return _speech_aux_collate_fn(batch, pad_id=self.pad_id)
        else:
            return _speech_collate_fn(batch, pad_id=self.pad_id, las_format=self.las_format)
...
class AudioToPhoneDataset(_AudioTextDataset):
    """
    ...
    def __init__(
        ...
        parser = parsers.PhoneParser(labels=labels, unk_id=unk_index, blank_id=blank_index, do_normalize=normalize, add_end_space=parser_add_end_space)
...
class AudioToBPEDataset(_AudioTextDataset):
    ...
    def __init__(
        ...
        if las_format:
            assert bos_id is None and eos_id is None
            bos_id = tokenizer.tokenizer.bos_id()
            eos_id = tokenizer.tokenizer.eos_id()
            assert bos_id >= 0 and eos_id >= 0
        ...
        aux_parser = aux_tokenizer.text_to_ids if aux_tokenizer else None
        ...
        super().__init__(
            ...
            data_dir=data_dir,
            dup_factor=dup_factor,
            parse_online=sampling_nbest_size is not None or aux_parser is not None,
            aux_parser=aux_parser,
            las_format=las_format,
        )
