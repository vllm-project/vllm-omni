...
def item_iter(
    ...
    for manifest_file in manifests_files:
        if not manifest_file.endswith('.csv'):
            ...
            df = pandas.read_csv(manifest_file, encoding='utf-8')
            for row in df.itertuples(index=False):
                k += 1
                item = __parse_csv_item(row, manifest_file)
                item['id'] = k
...
def __parse_item(line: str, manifest_file: str) -> Dict[str, Any]:
    ...
    if 'audio_filename' in item:
        ...
        item['audio_file'] = item.pop('audio')
    else:
        raise ValueError(
...
def __parse_csv_item(row, manifest_file: str) -> Dict[str, Any]:
    item = {'audio_file': row.wav_filename,
            'duration': (int(row.wav_filesize) - 44) / (16000 * 2),
            'offset': None,
            'speaker': None,
            'orig_sr': None}

    if hasattr(row, 'transcript'):
        item['text'] = row.transcript
