...
from my_scripts.fsq import FSQ
...
class VQCTCFinetuneModel(ASRModel):
    ...
    def __init__(self, cfg: DictConfig, trainer: Trainer = None):
        ...
        # self.pre_logits_bottleneck_layer = nn.Linear(self.encoder.feature_encoder.output_dim, cfg.quantizer.pre_logits_dim)
        #
        # self.quantizer = GumbelVectorQuantizer(
        #     dim=cfg.quantizer.pre_logits_dim, # quantizer_input_dim,
        ...
        levels = cfg.quantizer.levels
        self.pre_quant = nn.Linear(self.encoder.feature_encoder.output_dim, len(levels))
        self.quantizer = FSQ(levels=levels, l2_norm=cfg.quantizer.l2_norm, batch_norm=cfg.quantizer.batch_norm)
        ...
        try:
            ...
            with tempfile.TemporaryDirectory() as tmpdir:
                ...
                for test_batch in tqdm(temporary_datalayer, desc="Transcribing"):
                    ...
                    log_probs, logits_len, greedy_predictions, logits = \
                        self(input_signal=test_batch[0].to(device), input_signal_length=test_batch[1].to(device),
                             global_step=self.trainer.global_step)
    ...
    def forward(self, input_signal, input_signal_length, global_step):
        ...
        #
        # encoded = self.pre_logits_bottleneck_layer(encoded)
        # quantized_encoded, prob_ppl_loss, cur_temp, prob_ppl = self.quantizer(encoded)

        encoded = self.pre_quant(encoded)
        # quantized_encoded = self.quantizer.quantize(encoded)
        quantized_encoded = self.quantizer(encoded)
    ...
    def training_step(self, batch, batch_nb):
        ...
        log_probs, encoded_len, predictions, _ = \
            self(input_signal=signal, input_signal_length=signal_len, global_step=self.trainer.global_step)
        ...
        tensorboard_logs = {'train_loss': loss, 'ctc_loss': ctc_loss,
                            'learning_rate': self._optimizer.param_groups[0]['lr']}
    ...
    def validation_step(self, batch, batch_idx, dataloader_idx=0, decode_results=None):
        ...
        with torch.no_grad():
            ...
            log_probs, encoded_len, predictions, logits, = \
                self(input_signal=signal, input_signal_length=signal_len, global_step=self.trainer.global_step)
