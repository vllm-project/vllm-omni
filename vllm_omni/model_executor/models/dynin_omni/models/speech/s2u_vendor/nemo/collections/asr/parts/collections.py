...
class AudioText(_Collection):
    ...
    OUTPUT_TYPE = collections.namedtuple(
        typename='AudioTextEntity', field_names='id audio_file duration text_tokens offset text_raw speaker orig_sr aux_tokens',
    )
    ...
    def __init__(
        ...
        for id_, audio_file, duration, offset, text, speaker, orig_sr in zip(
            ...
            aux_tokens = None
            data.append(output_type(id_, audio_file, duration, text_tokens, offset, text, speaker, orig_sr, aux_tokens))
            if index_by_file_id:
    ...
    def get_item_online(self, i):
        ...
        if self.aux_parser:
            aux_tokens = self.aux_parser(data.text_raw)
            data = data._replace(aux_tokens=aux_tokens)
