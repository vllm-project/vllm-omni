...
from omegaconf import DictConfig, OmegaConf, open_dict, ListConfig, MISSING
...
from nemo.collections.asr.parts.perturb import RandomNoisePerturbation, AudioAugmentor
from nemo.utils import logging
from nemo.core.classes.modelPT import inspect, optim, hydra, prepare_lr_scheduler
...
@dataclass
class ST2VecVQCTCReconFinetuneModelConfig(ST2VecVQCTCFinetuneModelConfig):
    reconstruction_loss_weight: float = MISSING
    reconstructor: ReconstructorConfig = MISSING
...
class Reconstrutor(nn.Module):
    def __init__(self, cfg: DictConfig):
        ...
        self.generator = Projector(cfg.generator)
    ...
    def forward(self, h, h_len, target, target_len):
        # [B, D, T] => [B, T, D]
        ...
        pred = self.generator(h, length=h_len)
        ...
        return {'loss': loss, 'sample': sample}
...
class VQCTCReconFinetuneModel(ASRModel):
    """Todo: should modify"""
    ...
    def __init__(self, cfg: DictConfig, trainer: Trainer = None):
        ...
        self.reconstructor = Reconstrutor(self._cfg.reconstructor)

        self.quant_ppl_loss_weight = cfg.quant_ppl_loss_weight
        self.reconstruction_loss_weight = cfg.reconstruction_loss_weight
    ...
    def forward(self, input_signal, input_signal_length, global_step):
        ...
        # reconstruction from quantiszed vector!
        # [B, D，T] => [B, T, D]
        quantized_encoded = quantized_encoded.transpose(1, 2)
        specs, specs_len = self.encoder.wav2spec(input_signal=input_signal, length=input_signal_length)
        recon_dict = self.reconstructor(quantized_encoded, encoded_len, specs, specs_len)

        return log_probs, encoded_len, greedy_predictions, logits, prob_ppl_loss, cur_temp, prob_ppl, recon_dict

    def training_step(self, batch, batch_nb):
        signal, signal_len, transcript, transcript_len = batch

        log_probs, encoded_len, predictions, _, prob_ppl_loss, cur_temp, prob_ppl, recon_dict = \
        ...
        if self.quant_ppl_loss_weight != 0:
            ...
            loss += weighted_prob_ppl_loss
        if self.reconstruction_loss_weight != 0:
            reconstruction_loss = recon_dict['loss']
    ...
    def validation_step(self, batch, batch_idx, dataloader_idx=0, decode_results=None):
        signal, signal_len, transcript, transcript_len = batch
        with torch.no_grad():
            log_probs, encoded_len, predictions, logits, prob_ppl_loss, cur_temp, prob_ppl, recon_dict = \
                self(input_signal=signal, input_signal_length=signal_len, global_step=self.trainer.global_step)
            ctc_loss = self.ctc_loss(
                log_probs=log_probs, targets=transcript, input_lengths=encoded_len, target_lengths=transcript_len
            )
