...
"""EMOVA runtime module for dynin_omni speech tokenize/detokenize paths.

This file inlines the minimum runtime logic previously split across UVITS
submodules (commons/transforms/modules/attentions/models + text utilities).
Decode (U2S) runs locally, while encode (S2U) is loaded from the vendored
EMOVA tokenizer package when available.
"""
...
# text symbols from UVITS/text/symbols.py
_pad = "_"
_punctuation = ";:,.!?¡¿—…\"«»“” "
...
symbols = [_pad] + list(_punctuation) + list(_letters) + list(_letters_ipa) + [str(v) for v in range(4096)]
symbols_with_4096 = symbols
...
def _clean_text(text: str, cleaner_names):
    for name in cleaner_names:
        if name == "text_split":
            ...
            raise RuntimeError(
                f"Unsupported cleaner '{name}' in decode-only runtime. "
                "Expected 'text_split'."
            )
    return text
...
def text_to_sequence(text: str, cleaner_names):
    ...
    if isinstance(clean_text, str):
        ...
        iterable = clean_text
    return [_symbol_to_id[symbol] for symbol in iterable]
...
def _resolve_local_path(path: str) -> str:
    ...
    if p.is_absolute():
        return str(p)
    return str((_ASSET_ROOT / p).resolve())
...
commons = SimpleNamespace(
    ...
    rand_slice_segments=rand_slice_segments,
)
import torch
from torch.nn import functional as F
...
vae_utils = SimpleNamespace(gaussian_parameters=gaussian_parameters)
import copy
import math
import torch
from torch import nn
...
def _maximum_path(neg_cent, attn_mask):
    """Use compiled monotonic align when available.

    Inference code paths do not require this, but training forward does.
    Keep import-time optional to support environments where the extension
    cannot be built (e.g., Python 3.12).
    """
    if monotonic_align is not None:
        return monotonic_align.maximum_path(neg_cent, attn_mask)

    # Pure PyTorch fallback for environments where monotonic_align
    # extension cannot be built (e.g., Python 3.12).
    device = neg_cent.device
    out_dtype = neg_cent.dtype
    values = neg_cent.detach().to(dtype=torch.float32, device="cpu")
    mask = attn_mask.detach().to(dtype=torch.bool, device="cpu")
    bsz, t_y_max, t_x_max = values.shape
    path = torch.zeros((bsz, t_y_max, t_x_max), dtype=torch.float32, device="cpu")

    neg_inf = -1e9
    for b in range(bsz):
        y_len = int(mask[b, :, 0].sum().item())
        x_len = int(mask[b, 0, :].sum().item())
        if y_len <= 0 or x_len <= 0:
            continue

        v = values[b, :y_len, :x_len]
        dp = torch.full((y_len, x_len), neg_inf, dtype=torch.float32)
        backtrack = torch.zeros((y_len, x_len), dtype=torch.bool)
        dp[0, 0] = v[0, 0]

        for y in range(1, y_len):
            x_start = max(0, x_len + y - y_len)
            x_end = min(y, x_len - 1)
            for x in range(x_start, x_end + 1):
                stay = dp[y - 1, x]
                move = dp[y - 1, x - 1] if x > 0 else neg_inf
                if move >= stay:
                    dp[y, x] = v[y, x] + move
                    backtrack[y, x] = True
                else:
        ...
        x = x_len - 1
        path[b, y_len - 1, x] = 1.0
        for y in range(y_len - 1, 0, -1):
            if backtrack[y, x] and x > 0:
                x -= 1
            path[b, y - 1, x] = 1.0

    return path.to(device=device, dtype=out_dtype) * attn_mask.to(device=device, dtype=out_dtype)
...
class SynthesizerTrn(nn.Module):
    ...
    def forward(self, x, x_lengths, y, y_lengths, sid=None):
        ...
        with torch.no_grad():
            ...
            attn = _maximum_path(neg_cent, attn_mask.squeeze(1)).unsqueeze(1).detach()
...
def get_U2S_config_checkpoint_file(unit_type: str, language: str = "English") -> tuple[str, str]:
    del language
    if unit_type != "40ms_multilingual_8888_xujing_cosyvoice_FT":
        raise ValueError(f"Unsupported unit_type '{unit_type}'")
    config_file = "../configs/speech/config.json"
    if not (_ASSET_ROOT / config_file).resolve().exists():
        # Backward compatibility with the old location under models/speech/.
        config_file = "./config.json"
    checkpoint_file = "./G_322000.pth"
    return config_file, checkpoint_file


def load_U2S_config(model_config_file: str):
    hps = get_hparams_from_file(_resolve_local_path(model_config_file))
    hps.num_symbols = len(symbols_with_4096)
    return hps


def synthesis(unit_sequence, style_embedding, hps, net_g, output_wav_file: str = "output.wav"):
    device = next(net_g.parameters()).device
    logger.info("Generating audios on %s", device)
    with torch.no_grad():
...
def _load_external_s2u_runtime():
    # librosa/numba cache can fail under readonly site-packages. Force writable cache.
...
def get_S2U_ckpt_config_path(*args, **kwargs):
    if _imported_get_S2U_ckpt_config_path is None:
        _raise_s2u_unavailable("get_S2U_ckpt_config_path")
    return _imported_get_S2U_ckpt_config_path(*args, **kwargs)


def load_config(*args, **kwargs):
    if _imported_load_config is None:
        _raise_s2u_unavailable("load_config")
    return _imported_load_config(*args, **kwargs)


if _IMPORTED_VQCTCFinetuneModel is not None:
    VQCTCFinetuneModel = _IMPORTED_VQCTCFinetuneModel
else:
    class VQCTCFinetuneModel:
        def __init__(self, *args, **kwargs):
            del args, kwargs
            _raise_s2u_unavailable("VQCTCFinetuneModel")


def s2u_extract_unit_demo(*args, **kwargs):
    if _imported_s2u_extract_unit_demo is None:
        _raise_s2u_unavailable("s2u_extract_unit_demo")
    return _imported_s2u_extract_unit_demo(*args, **kwargs)
