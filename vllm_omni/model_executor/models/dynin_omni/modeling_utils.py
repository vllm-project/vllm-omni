...
except ImportError:  # Older huggingface_hub without shard helper.
    from huggingface_hub import create_repo

    def split_torch_state_dict_into_shards(
        state_dict, max_shard_size="10GB", filename_pattern="pytorch_model{suffix}.bin"
    ):
        class _StateDictSplit:
            def __init__(self, tensor_names, filename):
                self.filename_to_tensors = {filename: list(tensor_names)}
                self.tensor_to_filename = {name: filename for name in tensor_names}
                self.is_sharded = False
                self.metadata = {}

        filename = filename_pattern.format(suffix="")
        return _StateDictSplit(state_dict.keys(), filename)
    ...
    def _get_model_file(
        ...
        **_kwargs,
        ...
        if os.path.isdir(base):
            candidate = os.path.join(base, weights_name)
            if os.path.isfile(candidate):
                return candidate
        if os.path.isfile(pretrained_model_name_or_path):
            return pretrained_model_name_or_path
        raise OSError(f"Model file {weights_name} not found under {base}")
    ...
    def _get_checkpoint_shard_files(
        ...
        **_kwargs,
        ...
        if not index_path.is_file():
            raise OSError(f"Index file not found: {index_path}")
...
# Keep compatibility with diffusers versions that do not support the `variant` argument.
# Keep compatibility with signature changes in diffusers.
_LOAD_STATE_DICT_PARAMS = set(inspect.signature(load_state_dict).parameters)
_FETCH_INDEX_FILE_PARAMS = set(inspect.signature(_fetch_index_file).parameters)
# The meta loader signature has also changed across diffusers versions.
_LOAD_MODEL_DICT_PARAMS = set(inspect.signature(load_model_dict_into_meta).parameters)


def _load_state_dict_compat(checkpoint_file, **kwargs):
    filtered_kwargs = {k: v for k, v in kwargs.items() if k in _LOAD_STATE_DICT_PARAMS}
    return load_state_dict(checkpoint_file, **filtered_kwargs)


def _fetch_index_file_compat(**kwargs):
    filtered_kwargs = {k: v for k, v in kwargs.items() if k in _FETCH_INDEX_FILE_PARAMS}
    if "resume_download" in _FETCH_INDEX_FILE_PARAMS and "resume_download" not in filtered_kwargs:
        filtered_kwargs["resume_download"] = False
    return _fetch_index_file(**filtered_kwargs)


def _load_model_dict_into_meta_compat(model, state_dict, **kwargs):
    filtered_kwargs = {k: v for k, v in kwargs.items() if k in _LOAD_MODEL_DICT_PARAMS}
    result = load_model_dict_into_meta(model, state_dict, **filtered_kwargs)
    # Some versions return offload indexes instead of unexpected keys; normalize to a list for callers.
    if result is None:
        return []
...
class ModelMixin(torch.nn.Module, PushToHubMixin):
    ...
    def from_pretrained(cls, pretrained_model_name_or_path: str | os.PathLike | None, **kwargs: Any):
        r"""
        ...
        index_file = _fetch_index_file_compat(
            is_local=is_local,
