...
GENERIC_DIFFUSION_PROCESSOR = "vllm_omni.model_executor.stage_input_processors.diffusion.diffusion_stage_transition"
...
def _is_stage_payload(value: Any) -> bool:
    ...
    """
    if all(
        hasattr(value, attr) for attr in ("request_id", "boundary", "tensor_fields", "scalar_fields", "payload_version")
    ):
        return True
    try:
