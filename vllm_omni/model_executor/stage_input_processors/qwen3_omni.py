...
def _match_prefill_output(prefill_stage: Any | None, thinker_output: Any, index: int) -> Any | None:
    """Best-effort match of a decode output to its corresponding prefill output."""
    if prefill_stage is None or prefill_stage.engine_outputs is None:
        return None

    request_id = getattr(thinker_output, "request_id", None)
    if request_id is not None:
        for prefill_output in prefill_stage.engine_outputs:
            if getattr(prefill_output, "request_id", None) == request_id:
...
def thinker2talker(
    ...
    for i, thinker_output in enumerate(thinker_outputs):
        ...
        prefill_output = _match_prefill_output(prefill_stage, thinker_output, i)
        ...
        if prefill_output is not None:
            try:
                prefill_mm = prefill_output.outputs[0].multimodal_output
        ...
        def _tts(key: str) -> torch.Tensor:
            ...
            if val is None and prefill_output is not None:
                try:
...
def talker2code2wav(
    ...
    for talker_output in talker_outputs:
        ...
        if seq_len > max_seq_len:
            logger.warning(
                "Truncating talker codec from %d to %d frames (%d > %d flattened tokens) for code2wav",
                seq_len,
                max_seq_len,
                seq_len * num_quantizers,
                _CODE2WAV_MAX_PROMPT_LEN,
            )
            seq_len = max_seq_len

        # [num_quantizers, seq_len] -> [seq_len, num_quantizers] -> flat
        codec_codes = (
            codes_tensor[:, -seq_len:].to(torch.long).transpose(0, 1).cpu().to(torch.long).reshape(-1).tolist()
        )
