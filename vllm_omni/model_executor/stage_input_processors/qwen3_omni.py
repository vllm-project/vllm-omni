...
def _compute_talker_prompt_ids_length(info, device: torch.device | str = "cuda") -> int:
    ...
    thinker_sequences = (
        torch.tensor(info["thinker_sequences"], dtype=torch.long, device=device)
        .unsqueeze(0)
    ) # [1, T]

    input_ids = (
        torch.tensor(info["thinker_input_ids"], dtype=torch.long, device=device)
        .unsqueeze(0)
    ) # [1, T]
...
def thinker2talker(
    ...
    for i, thinker_output in enumerate(thinker_outputs):
        ...
        thinker_embeddings = (
            output.multimodal_output["0"]
            .detach()
            .to(device=device, dtype=torch.float)
        )

        thinker_hidden_states = (
            output.multimodal_output["24"]
            .detach()
            .to(device=device, dtype=torch.float)
        )
        info = {
            ...
            "tts_bos_embed": (
                output.multimodal_output.get("tts_bos_embed")
                .detach()
                .to(device=device, dtype=torch.float)
            ),
            "tts_eos_embed": (
                output.multimodal_output.get("tts_eos_embed")
                .detach()
                .to(device=device, dtype=torch.float)
            ),
            "tts_pad_embed": (
                output.multimodal_output.get("tts_pad_embed")
                .detach()
                .to(device=device, dtype=torch.float)
            ),
        }
