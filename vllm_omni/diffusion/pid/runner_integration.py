...
def init_pid_decoder_on(model: Any, od_config: Any) -> None:
    ...
    No-op when --enable-pid is off or the family is unregistered.
    """
...
def maybe_pid_passthrough(pipeline: Any, reqs: list[Any], od_config: Any) -> PidPassthrough | None:
    ...
    - Globally disabled: None; an explicit per-request ``pid_decode.enabled=True``
      raises (PiD weights are not lazily loaded; restart with --enable-pid).
    ...
    if decoder is None:
        if requested:
            raise RuntimeError(
                ...
                "pipeline was not configured with --enable-pid at startup (or its family is "
                "not registered in LATENT_FORMS). PiD weights are not lazily loaded on "
                "request; restart the service with --enable-pid to enable this feature."
            )
        return None
...
def stepwise_pid_active(pipeline: Any, state: Any) -> bool:
    ...
    if decoder is None:
        if override is not None and override.get("enabled") is True:
            raise RuntimeError(
                "PiD decode was requested per-request (pid_decode.enabled=True) but the "
                "pipeline was not configured with --enable-pid at startup. Restart the "
                "service with --enable-pid to enable this feature."
            )
        return False
    if sampling is None:
        return False
