...
class SequentialOffloader:
    ...
    def _to_cpu(self, module: nn.Module) -> None:
        ...
        if previous_device.type != "cpu":
            ...
            elif previous_device.type == "npu" and hasattr(torch, "npu"):
...
class LayerwiseOffloader:
    ...
    def __init__(
        ...
        assert current_omni_platform.is_cuda() or current_omni_platform.is_npu(), (
            "Layerwise offloading is only supported on CUDA or NPU devices for now"
        )
        ...
        # Use CUDA streams/events only when running on CUDA; NPU path uses synchronous copies.
        self._use_cuda_streams = current_omni_platform.is_cuda()
        self._copy_stream = torch.cuda.Stream() if self._use_cuda_streams else None

        # Per-layer synchronization primitive: set after H2D copy completes (CUDA only).
        self._prefetch_done: list[Any | None] = [None] * self.num_blocks
    ...
    def prefetch_layer(self, layer_idx: int, non_blocking: bool = True) -> None:
        ...
        if self._use_cuda_streams and self._copy_stream is not None:
        ...
        for idx in layers_to_fetch:
            ...
            if self._use_cuda_streams and self._copy_stream is not None:
                ...
                # On NPU (or other non-CUDA accelerators), fall back to synchronous copies.
                evt = None
                gpu_weights: dict[torch.dtype, torch.Tensor] = {}
                for dtype, cpu_weight in self.layer_cpu_weights[idx].items():
                    gpu_weight = cpu_weight.to(self.device)
                    gpu_weights[dtype] = gpu_weight
...
def apply_offload_hooks(
    ...
    *,
    ...
    if enable_cpu_offload:
        ...
        # Release allocator blocks when models are moved off the accelerator.
