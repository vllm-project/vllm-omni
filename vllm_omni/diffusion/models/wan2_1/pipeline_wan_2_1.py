...
from vllm_omni.diffusion.models.wan2_1.wan2_1_transformer import WanTransformer3DModel
from vllm_omni.diffusion.request import OmniDiffusionRequest


def get_wan21_post_process_func(
...
class Wan21Pipeline(nn.Module):
    """
    Wan 2.1 text-to-video pipeline for vLLM-Omni.

    This pipeline mirrors the vLLM-optimized Wan2.2 pipeline but keeps the
    single-denoiser flow and UniPC scheduler expected by Wan 2.1 checkpoints.
    """
    ...
    def forward(
        ...
        **kwargs,
        ...
        # Timesteps
        # UniPC stores `alphas_cumprod` as a torch.Tensor; ensure it is on CPU so
        # numpy conversion inside `set_timesteps` doesn't fail on CUDA tensors.
        if hasattr(self.scheduler, "alphas_cumprod") and torch.is_tensor(self.scheduler.alphas_cumprod):
            self.scheduler.alphas_cumprod = self.scheduler.alphas_cumprod.to("cpu")
