...
"""LTX-2.3 transformer variant.

This file intentionally stays separate from ``ltx2_transformer.py`` so
LTX-2.3 can align precision with the official implementation instead of
Diffusers without affecting LTX-2 behavior.
"""
...
def apply_interleaved_rotary_emb(x: torch.Tensor, freqs: tuple[torch.Tensor, torch.Tensor]) -> torch.Tensor:
    cos, sin = freqs
    # RoPE freqs are normally generated in the target hidden dtype. Keep this
    # guard for connector-provided freqs without paying a real copy on the hot path.
...
class LTX2VideoTransformerBlock(nn.Module):
    ...
    def forward(
        ...
        attn_hidden_states = self.attn1(
            ...
            all_perturbed=video_self_attention_all_perturbed,
        )
        ...
        attn_audio_hidden_states = self.audio_attn1(
            ...
            all_perturbed=audio_self_attention_all_perturbed,
        )
    ...
    def _build_sp_plan(rope_type: str) -> dict[str, Any]:
        ...
        return {
            "": {
                ...
                # Keep per-token modulation aligned with the sharded latents.
    ...
    def forward(
        ...
        **kwargs,  # Accept extra diffusers kwargs (isolate_modalities, perturbation_mask, etc.)
        ...
        video_cross_attn_rotary_emb = self.cross_attn_rope(
            video_coords[:, 0:1, :], device=hidden_states.device, out_dtype=hidden_states.dtype
        )
        audio_cross_attn_rotary_emb = self.cross_attn_audio_rope(
            audio_coords[:, 0:1, :], device=audio_hidden_states.device, out_dtype=audio_hidden_states.dtype
        )
        ...
        perturbation_kwargs = (attention_kwargs or {}).get("ltx23_perturbation_kwargs", {})
