...
from vllm.platforms import current_platform  # For vllm 0.19 RMSNorm below
...
class RMSNorm(CustomOp):
    """Copied from vllm/model_executor/layers/layernorm.py at v0.19.0~v0.19.1"""
    ...
    def __init__(
        ...
        if current_platform.is_rocm():
            ...
            self.rocm_norm_func = self.dispatch_rocm_rmsnorm_func(
                with_fused_add=False,
                dtype=weight_dtype,
                use_aiter=aiter_rmsnorm_enabled,
            )
            self.rocm_norm_func_with_add = self.dispatch_rocm_rmsnorm_func(
        ...
        if (
            ...
            if getattr(self._forward_method, "__func__", None) is getattr(self.forward_native, "__func__", None):
                ...
                logger.warning_once(
                    ...
                    custom_ops,  # pyright: ignore[reportArgumentType] # Error also present in original file
                )
    ...
    def forward_cuda(
        ...
        if (
            ...
            if self._can_view_as_2d(x):
                x_2d = x.view(-1, hidden_size)
                if self._is_oink_stride_compatible_2d(x_2d):
        ...
        if (
            ...
            orig_shape = x.shape
            hidden_size = orig_shape[-1]
            if self._can_view_as_2d(x) and self._can_view_as_2d(residual):
                x_2d = x.view(-1, hidden_size)
                ...
                # fused kernel.
                if (
                    self._is_oink_stride_compatible_2d(x_2d)
                    and self._is_oink_stride_compatible_2d(res_2d)
        ...
        if add_residual:
            return self.fused_add_rms_norm(x, residual, self.weight.data, self.variance_epsilon)
        else:
            return self.rms_norm(x, self.weight.data, self.variance_epsilon)
    ...
    def forward_hip(
        ...
        add_residual = residual is not None
        if add_residual:
            return self.rocm_norm_func_with_add(x, residual, self.weight.data, self.variance_epsilon)  # pyright: ignore[reportCallIssue] # Error also present in original file
        else:
            return self.rocm_norm_func(x, self.weight.data, self.variance_epsilon)  # pyright: ignore[reportCallIssue] # Error also present in original file
    ...
    @staticmethod
    def dispatch_rocm_rmsnorm_func(with_fused_add: bool, dtype: torch.dtype, use_aiter: bool = False):
        ...
        if with_fused_add:
            return RMSNorm.fused_add_rms_norm
        return RMSNorm.rms_norm
