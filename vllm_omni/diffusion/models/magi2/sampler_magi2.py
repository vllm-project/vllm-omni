...
class Magi2PreviewSampler(CFGParallelMixin):
    ...
    def denoise_step(
        ...
        *,
        ...
        video_cfg: float | torch.Tensor,
        audio_cfg: float | torch.Tensor,
    ) -> tuple[torch.Tensor, torch.Tensor]:
        """Execute exactly one joint video/audio denoising step.

        Includes input preparation, model prediction, CFG and scheduler updates;
        excludes request-level CFG precomputation and encoding/decoding.
        This is a sampling-step boundary, not just the transformer forward.
        """
