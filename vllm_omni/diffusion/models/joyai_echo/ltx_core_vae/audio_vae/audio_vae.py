...
from vllm_omni.diffusion.models.joyai_echo.ltx_core_vae.audio_vae.attention import AttentionType, make_attn
from vllm_omni.diffusion.models.joyai_echo.ltx_core_vae.audio_vae.causal_conv_2d import make_conv2d
from vllm_omni.diffusion.models.joyai_echo.ltx_core_vae.audio_vae.causality_axis import CausalityAxis
from vllm_omni.diffusion.models.joyai_echo.ltx_core_vae.audio_vae.downsample import build_downsampling_path
from vllm_omni.diffusion.models.joyai_echo.ltx_core_vae.audio_vae.ops import AudioProcessor, PerChannelStatistics
from vllm_omni.diffusion.models.joyai_echo.ltx_core_vae.audio_vae.resnet import ResnetBlock
from vllm_omni.diffusion.models.joyai_echo.ltx_core_vae.audio_vae.upsample import build_upsampling_path
from vllm_omni.diffusion.models.joyai_echo.ltx_core_vae.audio_vae.vocoder import Vocoder
from vllm_omni.diffusion.models.joyai_echo.ltx_core_vae.common.normalization import NormType, build_normalization_layer
from vllm_omni.diffusion.models.joyai_echo.ltx_core_vae.patchifiers import AudioPatchifier
from vllm_omni.diffusion.models.joyai_echo.ltx_core_vae.types import Audio, AudioLatentShape
...
class AudioEncoder(torch.nn.Module):
    ...
    def __init__(  # noqa: PLR0913
        ...
        **_ignore_kwargs,
        ...
        # ``PerChannelStatistics`` operates on the **post-patchify** packed
        # channel dim. ``AudioPatchifier.patchify`` rearranges
        # ``(b, c, t, f) -> (b, t, c*f)`` where ``c=z_channels`` and ``f`` is
        # the encoder-downsampled mel dim (``mel_bins / 2**(num_resolutions-1)``
        # for the standard ch_mult-driven freq downsampling). For JoyAI's
        # bundled config (``z_channels=8``, ``mel_bins=64``,
        # ``ch_mult=(1,2,4)``) this is ``8 * 16 = 128`` -- which happens to
        # equal the base channel multiplier ``ch``, but the *semantic* size
        # is the post-patchify packed dim, so we derive it explicitly.
        latent_mel_bins = mel_bins // (2 ** (len(ch_mult) - 1))
        self.per_channel_statistics = PerChannelStatistics(
            latent_channels=z_channels * latent_mel_bins,
        )
...
class AudioDecoder(torch.nn.Module):
    ...
    def __init__(  # noqa: PLR0913
        ...
        *,
        ...
        # Per-channel statistics for denormalizing latents. Sized to the
        # post-patchify packed dim ``z_channels * latent_mel_bins`` (see
        # the encoder constructor for the derivation). When ``mel_bins`` is
        # not provided at construction time, we fall back to the upstream
        # ``ch`` convention which is sized to match this packed dim by design.
        if mel_bins is not None:
            latent_mel_bins = mel_bins // (2 ** (len(ch_mult) - 1))
            packed_latent_channels = z_channels * latent_mel_bins
        else:
            packed_latent_channels = ch
        self.per_channel_statistics = PerChannelStatistics(latent_channels=packed_latent_channels)
        self.sample_rate = sample_rate
        self.mel_hop_length = mel_hop_length
        self.is_causal = is_causal
        self.mel_bins = mel_bins
