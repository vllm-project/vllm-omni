        ...
        **kwargs: Unpack[FlashAttentionKwargs],
        ...
        if past_key_values is not None:
            if use_cache:
                ...
                # transformers>=5.x: DynamicCache is not subscriptable; read the per-layer
                # cached tensors from cache.layers[idx].keys/.values instead of cache[idx][0/1].
    ...
    def forward(
        ...
        **kwargs: Unpack[FlashAttentionKwargs],
        ...
        attention_mask = create_causal_mask(
            # transformers>=5.x: kwarg renamed input_embeds->inputs_embeds and
            # cache_position was removed from the signature.
            config=self.config,
...
class Qwen3VLForConditionalGeneration(HFQwen3VLForConditionalGeneration):
    _checkpoint_conversion_mapping = {}
    # transformers>=5.x expects _tied_weights_keys as a dict {tied: source}
    # (matches upstream Qwen3VLForConditionalGeneration).
    ...
    def language_model(self):
        # transformers>=5.x moved the text model under `self.model.language_model`
        # and dropped the top-level `language_model` alias; restore it for compat.
        return self.model.language_model

    @property
    def visual(self):
        # transformers>=5.x moved the vision tower under `self.model.visual`;
        # restore the top-level alias for compat.
