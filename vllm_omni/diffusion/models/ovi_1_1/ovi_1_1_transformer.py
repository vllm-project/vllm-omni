...
class Ovi11FeedForward(nn.Module):
    """
    TP-enabled FeedForward network for Ovi 1.1.
...
class Ovi11RotaryPosEmbed(nn.Module):
    """
...
class Ovi11ImageEmbedding(nn.Module):
...
class Ovi11TimeTextImageEmbedding(nn.Module):
    ...
    def __init__(
        ...
        if image_embed_dim is not None:
            self.image_embedder = Ovi11ImageEmbedding(image_embed_dim, dim, pos_embed_seq_len=pos_embed_seq_len)
...
class Ovi11SelfAttention(nn.Module):
    """
    ...
    def forward(
        ...
        if rotary_emb is not None:
            ...
            query = apply_rotary_emb_ovi(query, freqs_cos, freqs_sin)
            key = apply_rotary_emb_ovi(key, freqs_cos, freqs_sin)
...
class Ovi11CrossAttention(nn.Module):
    """
...
class Ovi11TransformerBlock(nn.Module):
    """
    Transformer block for Ovi 1.1 model with self-attention, cross-attention, and FFN.
    ...
    def __init__(
        ...
        # 2. Cross-attention
        self.attn2 = Ovi11CrossAttention(
            dim=dim,
            num_heads=num_heads,
            head_dim=head_dim,
            eps=eps,
        ...
        # 3. Feed-forward
        self.ffn = Ovi11FeedForward(dim=dim, inner_dim=ffn_dim, dim_out=dim)
...
class Ovi11Transformer3DModel(nn.Module):
    """
    Optimized Ovi 1.1 Transformer model for video generation using vLLM layers.

    This is compatible with the WAN family architecture with Ovi-specific adaptations.
    ...
    Args:
        ...
        freq_dim: Time frequency dimension
        ffn_dim: Feed-forward network inner dimension
        num_layers: Number of transformer layers
        cross_attn_norm: Whether to apply norm on cross-attention
        eps: Epsilon for normalization
        image_dim: Image embedding dimension (for I2V)
        added_kv_proj_dim: Added KV projection dimension
        rope_max_seq_len: Maximum sequence length for RoPE
        pos_embed_seq_len: Position embedding sequence length
    """

    # Sequence Parallelism for Ovi 1.1 (following diffusers' _cp_plan pattern)
    #
    # The _sp_plan specifies sharding/gathering at module boundaries:
    # - rope_embed: Split both RoPE outputs (freqs_cos, freqs_sin) via split_output=True
    ...
    def __init__(
        ...
        self.condition_embedder = Ovi11TimeTextImageEmbedding(
            dim=inner_dim,
            time_freq_dim=freq_dim,
            time_proj_dim=6 * inner_dim,
        ...
        self.rope_embed = Ovi11RotaryPosEmbed(
            attention_head_dim=attention_head_dim,
            patch_size=patch_size,
        ...
        self.proj_out = nn.Linear(inner_dim, out_channels * patch_size[0] * patch_size[1] * patch_size[2])
    ...
    def forward(
        ...
        """
        Forward pass through Ovi 1.1 transformer.

        Args:
            hidden_states: Input tensor [B, C, T, H, W]
            timestep: Timestep tensor
            encoder_hidden_states: Text embeddings [B, seq_len, D]
        ...
        rotary_emb = self.rope_embed(hidden_states)

        # Patch embedding and flatten to sequence
        # (hidden_states is sharded at transformer_blocks.0 input by _sp_plan)
        ...
        # Use scale-shift modulation from temb
        if temb.ndim == 3:
            # [B, seq, D] -> need to pool for global modulation
            shift, scale = temb.chunk(2, dim=2)
            shift = shift.mean(dim=1, keepdim=True)
            scale = scale.mean(dim=1, keepdim=True)
        else:
            # [B, D] -> reshape for modulation
        ...
        # [B, seq_len, out_channels * p_t * p_h * p_w] -> [B, out_channels, T, H, W]
