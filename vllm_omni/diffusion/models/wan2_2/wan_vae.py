...
#
# Shared Wan2.2 VAE implementation used by model adapters that load the raw
# `Wan2.2_VAE.pth` checkpoint. Lance and NAVA both use this checkpoint and the
# same latent normalization / encode / decode math, but expose different adapter
# surfaces: Lance uses the 4-D BAGEL image path, while NAVA uses the 5-D video
# path for first-frame conditioning and final video decode.
#
# Keep the checkpoint-compatible implementation in this neutral Wan2.2 package
# instead of under `lance/` so NAVA does not depend on Lance as an
# implementation detail, and so future Wan2.2 VAE fixes land in one shared
# place.
#
# The implementation was originally ported for Lance from ByteDance Lance
# upstream (https://github.com/bytedance/Lance,
# modeling/vae/wan/{vae2_2.py,model.py}), whose VAE is itself based on the
# Alibaba Wan2.2 release. Upstream copyright:
#
#   Copyright (c) 2025 ByteDance Ltd. and/or its affiliates.
#   Copyright 2024-2025 The Alibaba Wan Team Authors. All rights reserved.
#   Licensed under the Apache License, Version 2.0.
#
# The checkpoint state dict is keyed for the upstream ``WanVAE_`` nn.Module here
# (not for diffusers' ``AutoencoderKLWan``), so this module preserves the
# upstream module structure to load ``Wan2.2_VAE.pth`` directly without
# state-dict surgery.
"""Shared Wan2.2 VAE for raw ``Wan2.2_VAE.pth`` checkpoints.

Lance and NAVA both use the same Wan2.2 VAE checkpoint and latent
normalization math. This module keeps that checkpoint-compatible implementation
under the neutral ``wan2_2`` package so model adapters can share the VAE without
depending on each other's model directories.

The module owns the ``WanVAE_`` port that matches the checkpoint keys and the
``Wan22VAE`` adapter that exposes both image-style and video-style surfaces.
"""
...
class Wan22VAE(nn.Module):
    """Wan2.2 VAE adapter for raw ``Wan2.2_VAE.pth`` checkpoints.

    The shared implementation exposes two surfaces used by current model
    adapters:

    - ``encode(BCHW)`` and ``decode(BC_zHW)`` for image-style adapters such as
      Lance, where each image is treated as a one-frame video clip.
    - ``encode_video(BCTHW)`` and ``decode_video(BC_tHW)`` for video adapters
      such as NAVA, where the temporal latent axis is preserved.

    Construction is lazy: the heavy ``WanVAE_`` and checkpoint are materialized
    only on first use when requested. Once built, the inner module is registered
    as a submodule so ``parameters()`` and ``to(device)`` follow PyTorch module
    semantics.
    """
    ...
    def __init__(
        ...
        *,
        ...
        self._dtype = dtype
        self._scale_dtype = scale_dtype or dtype
    ...
    def _ensure_built(self) -> None:
        ...
        mean = torch.tensor(_WAN22_LATENT_MEAN, dtype=self._scale_dtype, device=device)
        std = torch.tensor(_WAN22_LATENT_STD, dtype=self._scale_dtype, device=device)
        inv_std = 1.0 / std
    ...
    def _module_input_dtype(self, fallback: torch.dtype) -> torch.dtype:
        return self._autocast_dtype or fallback
    ...
    def encode_video(self, video: torch.Tensor, *, use_sample: bool = True) -> torch.Tensor:
        ...
        self._ensure_built()
        video = video.to(self._module_input_dtype(self.model.encoder.conv1.weight.dtype))
        ctx = (
            torch.amp.autocast("cuda", dtype=self._autocast_dtype)
            if self._autocast_dtype is not None and video.device.type == "cuda"
    ...
    def decode_video(self, latent: torch.Tensor) -> torch.Tensor:
        ...
        self._ensure_built()
        latent = latent.to(self._module_input_dtype(self.model.decoder.conv1.weight.dtype))
        ctx = (
            torch.amp.autocast("cuda", dtype=self._autocast_dtype)
            if self._autocast_dtype is not None and latent.device.type == "cuda"
            else contextlib.nullcontext()
        )
        with ctx:
    ...
    def encode(self, padded_images: torch.Tensor) -> torch.Tensor:
        ...
        if padded_images.dim() == 4:
            ...
            raise ValueError(f"Wan22VAE.encode expects 4-D BCHW or 5-D BCTHW, got {tuple(padded_images.shape)}")
    ...
    def decode(self, latent: torch.Tensor) -> torch.Tensor:
        ...
        if latent.dim() == 4:
            ...
            raise ValueError(f"Wan22VAE.decode expects 4-D BCHW or 5-D BCTHW, got {tuple(latent.shape)}")
...
def build_wan22_vae(vae_path: str, dtype: torch.dtype = torch.bfloat16, device=None) -> Wan22VAE:
    """Convenience factory: lazy-construct a :class:`Wan22VAE` adapter."""
    return Wan22VAE(vae_path=vae_path, dtype=dtype, device=device)


__all__ = ["Wan22VAE", "WanVAE_", "build_wan22_vae"]
