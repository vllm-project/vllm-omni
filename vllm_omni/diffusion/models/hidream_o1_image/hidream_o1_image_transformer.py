...
# Adapted from https://github.com/huggingface/transformers/blob/main/ and
# https://github.com/HiDream-ai/HiDream-O1-Image/blob/main/models/qwen3_vl_transformers.py
...
USE_BF16_ROPE = os.environ.get("USE_BF16_ROPE", "0") == "1"
    ...
    def forward(
        ...
        **kwargs,
        ...
        if self.config._attn_implementation == "flash_attention_2":
            # FA2 needs cu_seqlens for variable-length attention.
...
def _compute_default_rope_parameters(config: Qwen3VLTextConfig, device=None, **_kw):
    """Standard non-scaled RoPE inv_freq, compatible with transformers 4.57 and 5.x.

    Upstream HiDream-O1 pins ``transformers==4.57.1`` and looks up
    ``ROPE_INIT_FUNCTIONS["default"]``. Two breaking changes from 4.x -> 5.x
    make that path unusable inside vLLM-Omni (which allows ``transformers>=4.56``,
    currently resolving to 5.x on CUDA):
    1. ``"default"`` was removed from ``ROPE_INIT_FUNCTIONS`` (moved to an
       instance method on the concrete ``RotaryEmbedding`` class).
    2. ``rope_theta`` was moved off the top-level config attribute into the
       ``rope_scaling`` / ``rope_parameters`` dict (which are property aliases
       in 5.x).
    """
...
class Qwen3VLTextRotaryEmbedding(nn.Module):
    ...
    def compute_default_rope_parameters(self, config=None, device=None):
        """Instance-method delegate to the module-level `_compute_default_rope_parameters`.

        Required by `transformers>=5.x` `_init_weights` (modeling_utils.py: 2510), which
        calls `module.compute_default_rope_parameters(config, device)` on every
        `RotaryEmbedding` to re-init `inv_freq` after standard weight init. Upstream
        HiDream-O1 (pinned to `transformers==4.57`) does not carry this method because
        the 4.x `_init_weights` never looked for it; on `transformers>=5.x` its absence
        crashes `.post_init()` on non-meta devices (meta init skips this path).
        """
        cfg = config if config is not None else self.config
        return _compute_default_rope_parameters(cfg, device)
    ...
    def forward(self, x, position_ids):
        if self.original_inv_freq.is_meta and not self.inv_freq.is_meta:
            self.original_inv_freq = self.inv_freq

        # Qwen3VL has different position ids for the grids, so expand inv_freq to shape (3, ...).
...
class Qwen3VLTextAttention(nn.Module):
    def __init__(self, config: Qwen3VLTextConfig, layer_idx: int):
        ...
        # Head-dim RMSNorm on Q/K (unlike OLMo). No reshape needed after q_norm.
    ...
    def forward(self, hidden_states: torch.Tensor, grid_thw: torch.Tensor, **kwargs) -> torch.Tensor:
        ...
        Args:
            hidden_states (`torch.Tensor` of shape `(seq_len, hidden_size)`):
                Final hidden states of the model.
            grid_thw (`torch.Tensor` of shape `(num_images_or_videos, 3)`):
                Temporal / height / width of feature shape of each image in LLM.
        """
        ...
        cu_seqlens = torch.repeat_interleave(grid_thw[:, 1] * grid_thw[:, 2], grid_thw[:, 0]).cumsum(
            dim=0,
            # FA2 requires cu_seqlens_q dtype int32; torch.onnx.export requires same dtype as grid_thw.
...
class Qwen3VLTextModel(Qwen3VLPreTrainedModel):
    ...
    def forward(
        ...
        cache_position: torch.LongTensor | None = None,
        # DeepStack (see https://arxiv.org/abs/2406.04334).
        visual_pos_masks: torch.Tensor | None = None,
        deepstack_visual_embeds: list[torch.Tensor] | None = None,
        return_mid_results_layers: list | None = None,
        **kwargs: Unpack[FlashAttentionKwargs],
        ...
        for layer_idx, decoder_layer in enumerate(self.layers):
            if self.gradient_checkpointing and torch.is_grad_enabled():
                # Use HF's _gradient_checkpointing_func (use_reentrant=False from enable()).
            ...
            # Inject DeepStack visual features into the first several layers.
...
class BottleneckPatchEmbed(nn.Module):
    ...
    def initialize_weights(self):
        nn.init.xavier_uniform_(self.proj1.weight.data.view(self.proj1.weight.data.shape[0], -1))
        nn.init.xavier_uniform_(self.proj2.weight.data.view(self.proj2.weight.data.shape[0], -1))
...
class FinalLayer(nn.Module):
    def __init__(self, config, hidden_size: int, patch_size: int, out_channels: int):
        super().__init__()
...
class TimestepEmbedder(nn.Module):
    def __init__(self, config, hidden_size: int, frequency_embedding_size: int = 256):
        super().__init__()
...
class Qwen3VLModelOutputWithPast(ModelOutput):
    ...
    x_pred: torch.FloatTensor | None = None
    mid_results: list | None = None
    # Concatenated conditioning image tokens (I2I); returned for caching across denoise steps.
    cond_image_embeds: torch.FloatTensor | None = None
    cond_deepstack_image_embeds: torch.FloatTensor | None = None
...
class Qwen3VLModel(Qwen3VLPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        # Pixel-DiT configuration. patch_size=32 is HiDream-O1's canonical choice
        # (fixed 32x32 pixel patches, matches upstream HiDream-O1-Image inference).
        self.patch_size = 32
        self.in_channels = 3
        hidden_size = config.text_config.hidden_size
        bottleneck_dim = hidden_size // 4

        # Timestep -> hidden conditioning. t_embedder2 is upstream-declared but
        # explicitly None in HiDream-O1-Image (an unused variant slot; kept for
        # checkpoint-name parity with the upstream fork).
        self.t_embedder1 = TimestepEmbedder(self.config, hidden_size)
        self.t_embedder2 = None

        # Pixel patch <-> hidden bridges.
        ...
        # <|tms|> (timestep marker) token id in HiDream-O1's expanded Qwen3-VL vocab.
        ...
        if input_ids is not None and (image_grid_thw is not None or video_grid_thw is not None):
            ...
            for i, input_ids in enumerate(total_input_ids):
                ...
                for _ in range(image_nums + video_nums):
                    ...
                    # t_index is always 0 because llm_grid_t is always 1 (temporal info encoded via timestamp tokens).
...
class Qwen3VLForConditionalGeneration(Qwen3VLPreTrainedModel, GenerationMixin):
    ...
    # extra kwargs from the training loop.
    accepts_loss_kwargs = False
    # See Qwen3VLPreTrainedModel comment: explicit config_class needed under
    # `from __future__ import annotations` (PEP 563 lazy annotations).
    # Without this line, transformers 5.x errors during
    # `HiDreamO1ImageTransformer.from_pretrained(...)` with
    ...
    # `model.language_model` on the vanilla Qwen3-VL top-level class works
    # transparently on the LM-head wrapper. (BC-compat).
    @property
    ...
    def prepare_inputs_for_generation(
        ...
        **kwargs,
        ...
        # Overwritten: in specific circumstances (post-first-step continuation)
        # we don't want to forward image inputs to the model.
    ...
    def _expand_inputs_for_generation(
        ...
        **model_kwargs,
        ...
        # Overwritten: supports expanding visual tensors that lack an explicit
        # batch dim (pixel_values, image_grid_thw, pixel_values_videos,
        # video_grid_thw, second_per_grid_ts). pixel_values.shape[0] is
        # sum(seqlen_images for samples), image_grid_thw.shape[0] is
        # sum(num_images for samples).
        if expand_size == 1:
...
HiDreamO1ImageTransformer = Qwen3VLForConditionalGeneration
