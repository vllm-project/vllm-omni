...
def prepare_ernie_text_encoder_fp8(
    encoder: Mistral3Model, quant_config: QuantizationConfig | None, device: torch.device
) -> int:
    """Prepare ERNIE-Image Mistral decoder projections for online FP8 loading."""
    ...
    if (
        ...
        raise ValueError("ERNIE-Image text_encoder supports dynamic online FP8 from an unquantized checkpoint only.")
    ...
    for name in linear_names:
        ...
        if dtype not in (torch.bfloat16, torch.float16):
            raise ValueError("ERNIE-Image text_encoder FP8 requires BF16 or FP16 weights.")
        ...
        with torch.device(device), set_default_torch_dtype(dtype):
            replacement = ReplicatedLinear(
                ...
                params_dtype=dtype,
                quant_config=config,
                prefix=f"text_encoder.language_model.layers.{name}",
                return_bias=False,
                disable_tp=True,
            )
