...
def prepare_helios_text_encoder_fp8(
    encoder: UMT5EncoderModel, quant_config: QuantizationConfig | None, device: torch.device
) -> int:
    """Prepare Helios UMT5 feed-forward inputs for online FP8 loading."""
    ...
    if (
        ...
        raise ValueError("Helios text_encoder supports dynamic online FP8 from an unquantized checkpoint only.")
    layers = encoder.encoder.block
    replaced = 0
    for name, layer in list(layers.named_modules()):
        if not isinstance(layer, nn.Linear) or name.rsplit(".", 1)[-1] not in ("wi_0", "wi_1"):
            continue
        dtype = layer.weight.dtype
        if dtype not in (torch.bfloat16, torch.float16):
            raise ValueError("Helios text_encoder FP8 requires BF16 or FP16 weights.")
        ...
        with torch.device(device), set_default_torch_dtype(dtype):
            replacement = ReplicatedLinear(
                ...
                params_dtype=dtype,
                quant_config=config,
                prefix=f"text_encoder.encoder.block.{name}",
                return_bias=False,
                disable_tp=True,
            )
