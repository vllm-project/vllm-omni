...
class StageComponentSpec:
    ...
    def union(self, other: StageComponentSpec) -> StageComponentSpec:
        ...
        """
        return StageComponentSpec(**{f.name: (getattr(self, f.name) or getattr(other, f.name)) for f in fields(self)})
        ...
        if role == DENOISE:
            if not sources:
                ...
                if not (upstream_roles & {ENCODE, DENOISE}):
                    errors.append(
                        f"Denoise stage {sid} has no upstream encode/denoise source "
                        f"(got roles {sorted(upstream_roles)})."
                    )
                if len(sources) > 1:
                    errors.append(
