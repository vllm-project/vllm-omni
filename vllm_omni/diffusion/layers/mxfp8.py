...
@torch.library.custom_op("vllm_omni::mxfp8_quantize_project", mutates_args=())
def mxfp8_quantize_project(
    x: torch.Tensor,
    weight: torch.Tensor,
    scale: torch.Tensor,
) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
    """Project one weight slice and expose quantized X for another consumer.

    Returning both payload and scale makes the data dependency explicit across
    compile boundaries. No tensor is retained in a process-global registry.
    """
    q, activation_scale = mxfp8_quantize_swizzled(x)
    out = mxfp8_scaled_mm(q, weight, activation_scale, scale)
    return out, q, activation_scale


@mxfp8_quantize_project.register_fake
def _mxfp8_quantize_project_fake(x, weight, scale):
    return (
        x.new_empty((x.shape[0], weight.shape[0])),
        x.new_empty(x.shape, dtype=torch.float8_e4m3fn),
        x.new_empty((_scale_numel(*x.shape),), dtype=torch.uint8),
    )
