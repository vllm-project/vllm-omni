...
class _BaseScheduler(SchedulerInterface):
    ...
    def block_request(self, request_id: str) -> bool:
        ...
        self._blocked.add(request_id)
        self._request_states[request_id].status = DiffusionRequestStatus.BLOCKED
        return True

    def unblock_request(self, request_id: str) -> bool:
        """Move a BLOCKED request to WAITING."""

        if request_id not in self._blocked:
            return False
        self._blocked.discard(request_id)
        self._waiting.append(request_id)
        self._request_states[request_id].status = DiffusionRequestStatus.WAITING
        return True
    ...
    def _finish_requests(
        ...
        for request_id, status in statuses.items():
            ...
            if request_id in self._blocked:
                blocked_to_remove.add(request_id)
