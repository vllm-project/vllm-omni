...
# =============================================================================
# Debug tracing for duplex realtime path
# =============================================================================

_TRACE_TAG = "[DTRACE]"
_TRACE_LOGGER = logging.getLogger("vllm_omni.dtrace")


def _install_duplex_trace():
    if not os.environ.get("VLLM_OMNI_DTRACE"):
        return
    ...
    if not _TRACE_LOGGER.handlers:
        ...
        h.setFormatter(logging.Formatter(f"%(asctime)s {_TRACE_TAG} %(message)s"))
        _TRACE_LOGGER.addHandler(h)

    _duplex_prefix = "vllm_omni/experimental/fullduplex"
    ...
    def _trace_calls(frame, event, arg):
        ...
        if _duplex_prefix not in filename:
            return _trace_calls
        tid = threading.current_thread().name
        short_file = filename.rsplit("fullduplex/", 1)[-1]
        _TRACE_LOGGER.debug("%s:%d %s() [%s]", short_file, co.co_firstlineno, name, tid)
        return _trace_calls

    sys.settrace(_trace_calls)
    threading.settrace_all_threads(_trace_calls)
    _TRACE_LOGGER.info("Duplex function tracing enabled")


_install_duplex_trace()
