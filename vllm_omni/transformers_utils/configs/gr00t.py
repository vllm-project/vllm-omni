...
Mirrors the ``config.json`` shipped with the ``nvidia/GR00T-N1.7-3B``
checkpoint.  Every field is declared explicitly so checkpoint-loading
paths (model + processor) can rely on them.

The GR00T robot config is flat (no nested text_config / vision_config).  At
init time we overlay the Qwen3-VL backbone (Cosmos-Reason2-2B) sub-configs
onto this config so that any consumer that inspects `text_config` /
`vision_config` / `vocab_size` etc. resolves the backbone fields without
having to crack the GR00T checkpoint again.
"""
...
def _load_backbone_config(model_name: str) -> PretrainedConfig | None:
    """Load the Qwen3-VL backbone config given the GR00T ``model_name``.

    ``model_name`` accepts either an HF repo id (e.g.
    ``nvidia/Cosmos-Reason2-2B``, which is gated and needs ``HF_TOKEN`` or
    ``huggingface-cli login``) or a local checkout path.
    """
    if not model_name:
        return None
    try:
        ...
        logger.warning(
            ...
            "If this is a gated HF repo, run `huggingface-cli login` (or "
            "set HF_TOKEN) so transformers can authenticate; alternatively "
            "set `model_name` to a local checkout path.",
            model_name,
            exc,
        )
        return None
...
class Gr00tN1d7Config(PretrainedConfig):
    ...
    def __init__(
        ...
        # Image / processor (kept so to_dict roundtrips cleanly)
...
AutoConfig.register("Gr00tN1d7", Gr00tN1d7Config, exist_ok=True)
