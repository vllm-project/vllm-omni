...
def print_text_metrics(
    task_type, selected_percentile_metrics, metrics: MultiModalsBenchmarkMetrics
):
...
def print_audio_metrics(
    selected_percentile_metrics, metrics: MultiModalsBenchmarkMetrics
):
    print("{s:{c}^{n}}".format(s=" Audio Result ", n=50, c="="))
    print(
        "{:<40} {:<10.2f}".format(
            "Total audio duration generated(s):", metrics.total_audio_duration_s
        )
    )
    print(
        "{:<40} {:<10}".format(
            "Total audio frames generated:", metrics.total_audio_frames
        )
    )
    print(
        "{:<40} {:<10.2f}".format(
            "Audio throughput(audio duration/s):", metrics.audio_throughput
        )
    )
    for metric in selected_percentile_metrics:
...
def process_one_metric(
    ...
    print(
        f"{f'Mean {metric_attribute_name.upper()}{unit_suffix}:':<40} {mean_value:<10.2f}"
    )

    median_attr_name = f"median_{metric_attribute_name}{suffix}"
    median_value = getattr(metrics, median_attr_name, 0.0)
    print(
        f"{f'Median {metric_attribute_name.upper()}{unit_suffix}:':<40} {median_value:<10.2f}"
    )
...
def calculate_metrics(
    ...
    if goodput_config_dict:
        ...
        if "audio_ttft" in goodput_config_dict:
            ...
            slo_values.append(
                goodput_config_dict["audio_ttft"] / MILLISECONDS_TO_SECONDS_CONVERSION
            )
