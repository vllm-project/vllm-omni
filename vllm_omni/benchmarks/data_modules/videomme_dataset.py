...
def _resolve_hf_cache_snapshot(path: Path) -> Path:
    ...
    if ref.is_file():
        try:
            ...
            revision = ""
        # An empty or unreadable ref must not resolve to ``snapshots`` itself, which holds
        # no dataset files; fall through to the newest revision below.
...
def ensure_videomme_hub_root(repo_id: str) -> Path:
    ...
    Hub ids trigger ``hf_api().snapshot_download``; callers then extract videos via
...
def ensure_videomme_videos_extracted(root: Path) -> Path:
    ...
    if next(_iter_video_files(video_dir), None) is None:
        # Marking an empty tree as extracted would make every later request log
        # "video not found" and yield a run with zero samples.
        raise FileNotFoundError(f"No Video-MME videos under {root}: expected video/*.mp4 or videos_chunked_*.zip")
...
class VideoMMEDataset(BenchmarkDataset):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if pack_mode == "minicpm-interleave" and duration_filter != "short":
            logger.warning(
                "Video-MME 'minicpm-interleave' ports the OmniEvalKit videomme_short recipe. Beyond the "
                "short bucket, frames are spread across the whole clip and each paired audio segment covers "
                "the gap between them (tens of seconds on medium/long videos), which inflates the audio "
    ...
    def sample(
        ...
        **kwargs,
        ...
        if not sampled and num_requests > 0:
            # Oversampling an empty list raises a bare IndexError; the usual cause is a
            # video_dir that holds none of the videoIDs referenced by the QA rows.
            raise ValueError(
                f"Video-MME produced no requests from {seen} QA rows "
    ...
    def _frame_cache_dir(self, video_id: str, *, include_audio: bool) -> Path | None:
        """Where the on-disk frame dump lives, or ``None`` when there is no ``video_dir`` to host it."""
        if self.video_dir is None:
            return None
