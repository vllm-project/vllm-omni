...
def _resize(items: list[dict[str, Any]], num_prompts: int) -> list[dict[str, Any]]:
...
def _load_random_prompts(args: Namespace) -> list[dict[str, Any]]:
    ...
    return [{"prompt": f"Random prompt {i} for benchmarking diffusion models"} for i in range(num_prompts)]
...
def _load_custom_prompts(args: Namespace) -> list[dict[str, Any]]:
    ...
    path = getattr(args, "dataset_path", None)
    if not path:
        raise ValueError("--dataset-path is required when using --diffusion-dataset custom")
    items: list[dict[str, Any]] = []
    with open(path, encoding="utf-8") as f:
...
def load_diffusion_samples(args: Namespace) -> list[SampleRequest]:
    ...
    if loader is None:
        raise ValueError(f"Unknown diffusion dataset: {dataset_name!r}. Choose from {sorted(_DATASET_LOADERS)}")
