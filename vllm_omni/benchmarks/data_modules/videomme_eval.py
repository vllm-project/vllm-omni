...
def compute_videomme_accuracy_metrics(
    ...
    *,
    ...
    (``_validate_videomme``) additionally rejects HTTP failures, duplicate question
    IDs, skipped source rows and missing gold answers. Direct
    ...
    ``videomme_skipped_rows`` counts source rows skipped during sampling (for
    example, missing or undecodable media), before any oversampling.
    """
    ...
    for req, out in zip(input_requests, outputs, strict=True):
        ...
        is_correct = pred is not None and pred.upper() == (gold_norm or gold_raw).upper()
    ...
    result: dict[str, Any] = {
        ...
        "videomme_skipped_rows": max(req.videomme_skipped_rows for req in input_requests),
    }
...
def print_videomme_accuracy_summary(metrics: dict[str, Any]) -> None:
    ...
    for label, key in (
        ...
        ("Skipped source rows:", "videomme_skipped_rows"),
    ):
