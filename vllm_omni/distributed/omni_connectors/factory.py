...
class OmniConnectorFactory:
    ...
    def create_stage_connectors(
        ...
        *,
        ...
        if receive is not None and send is not None and receive.name == "NixlConnector":
            # A NIXL middle worker uses one connector for both directions: it
            # reads from the upstream endpoint and binds its own listener so
            # the downstream stage can read its outgoing payloads.
            if send.name != "NixlConnector":
                raise ValueError("A NIXL middle stage requires NIXL on its outgoing edge")
            extra = dict(receive.extra)
            extra["zmq_port"] = send.extra["zmq_port"]
            if send.extra.get("host") is not None:
                extra["host"] = send.extra["host"]
            connector = cls.create_connector(ConnectorSpec(receive.name, extra))
    ...
    def materialize_connector_spec(
        ...
        if edge.spec.name in TRANSFER_ENGINE_CONNECTOR_NAMES:
            ...
            if role == "receiver":
                # A receive-only edge reads from the producer's socket and has
                # nothing to bind; keeping a bind port here makes a connector
                # that listens (NIXL) collide with the producer it reads from.
                # A middle stage gets its bind port from the outbound edge.
                extra.pop("zmq_port", None)
            else:
                extra["zmq_port"] = bind_port
...
# Dual-role operation is not yet validated for NIXL, so it keeps one connector
# per direction.
