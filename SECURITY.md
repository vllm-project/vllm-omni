...
Please report security issues privately using [the vulnerability submission form](https://github.com/vllm-project/vllm-omni/security/advisories/new).

## Issue triage

Reports will be triaged by the vLLM-Omni maintainers and coordinated with the upstream [vLLM vulnerability management team](https://docs.vllm.ai/en/latest/contributing/vulnerability_management.html) when applicable.

## Threat model

vLLM-Omni extends vLLM with multimodal capabilities (vision, speech). Please see the [vLLM Security Guide](https://docs.vllm.ai/en/latest/usage/security.html) for information on vLLM's security assumptions and recommendations.
...
For certain security issues of CRITICAL, HIGH, or MODERATE severity level, we may prenotify certain organizations or vendors that ship vLLM-Omni. The purpose of this prenotification is to allow for a coordinated release of fixes for severe issues.
...
* Organizations and vendors who either ship or use vLLM-Omni, are eligible to join the prenotification group if they meet at least one of the following qualifications
    * Substantial internal deployment leveraging the upstream vLLM-Omni project.
    * Established internal security teams and comprehensive compliance measures.
    * Active and consistent contributions to the upstream vLLM-Omni project.
