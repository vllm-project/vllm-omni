...
#
# Usage:
#   bash tools/ready/run_ready_jobs.sh
#   REPO_ROOT=/path/to/vllm-omni bash tools/ready/run_ready_jobs.sh --model-type omni --dry-run
#   YML=/path/to/vllm-omni/.buildkite/test-ready.yml bash tools/ready/run_ready_jobs.sh
#
...
if [[ -n "${YML:-}" && -n "${REPO_ROOT:-}" ]]; then
  ...
  if ! REPO_ROOT="$(_derive_repo_root_from_yml "${YML}")"; then
    echo "Could not derive REPO_ROOT from YML=${YML} (expected file at <repo>/.buildkite/test-ready.yml)." >&2
...
LOG_DIR="${LOG_DIR:-${REPO_ROOT}/logs/ready_jobs}"
