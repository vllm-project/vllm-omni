...
def main() -> None:
    ...
    if result.final_output_type != "image":
        raise ValueError(f"expected diffusion image output, got {result.final_output_type!r}")
    if result.num_images != 1:
        raise ValueError(f"expected exactly one image, got num_images={result.num_images}")
