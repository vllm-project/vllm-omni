...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Gradio demo for FLUX 2 offline inference.")
    parser.add_argument("--model", default="black-forest-labs/FLUX.2-dev", help="Diffusion model name or local path.")
    parser.add_argument(
        "--height",
        type=int,
        default=1024,
    ...
    parser.add_argument("--default-prompt", default="a serene mountain landscape at sunset", help="Initial prompt shown in UI.")
    parser.add_argument("--default-seed", type=int, default=42, help="Initial seed shown in UI.")
    parser.add_argument("--default-guidance-scale", type=float, default=4.0, help="Initial guidance scale shown in UI.")
    parser.add_argument(
        "--num_inference_steps",
        type=int,
        default=50,
    ...
    parser.add_argument("--port", type=int, default=7863, help="Port for Gradio `launch`.")
...
def build_demo(args: argparse.Namespace) -> gr.Blocks:
    ...
    with gr.Blocks(
        title="vLLM-Omni FLUX 2 Web Demo",
        css="""
        ...
        gr.Markdown("# vLLM-Omni FLUX 2 Web Demo")
        ...
        with gr.Row():
            with gr.Column(scale=1, elem_classes="left-column"):
                ...
                guidance_input = gr.Number(label="Guidance Scale", value=args.default_guidance_scale)
        ...
        generate_btn.click(
            fn=run_inference,
            inputs=[prompt_input, seed_input, guidance_input, aspect_dropdown, steps_input, num_images],
            outputs=gallery,
        )
