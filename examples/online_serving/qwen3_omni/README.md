...
- `--voice` (or `-V`): TTS speaker/voice for audio output when requesting audio (e.g. `ethan`, `chelsie`). Omit to use the model default. Example: `--voice chelsie`
...
Use the `--voice` argument when generating audio:
...
response = client.chat.completions.create(
    ...
    messages=[{"role": "user", "content": "Say hello in one sentence."}],
    modalities=["audio"],
    extra_body={"voice": "chelsie"}
)
# Audio uses the specified voice
