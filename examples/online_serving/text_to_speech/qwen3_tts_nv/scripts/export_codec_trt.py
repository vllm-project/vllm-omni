...
def _make_runtime_zero(np_type, base_name):
    ...
    seed = gs.Constant(name=f"{base_name}_seed",
                       values=np.array(1, dtype=np_type))
    zero = gs.Variable(name=f"{base_name}_zero", dtype=np_type, shape=())
    sub = gs.Node(op="Sub", inputs=[seed, seed], outputs=[zero],
...
def _make_add_zero_barrier(tensor, zero_var, name):
    ...
    add = gs.Node(op="Add", inputs=[tensor, zero_var],
                  outputs=[new_tensor], name=name)
...
def apply_trt_fusion_barrier(
    ...
    if in_tensor.dtype is None:
        raise RuntimeError(
            f"cannot insert barrier on {in_tensor.name!r}: dtype unknown")
    ...
    zero_sub, zero_var = _make_runtime_zero(
        np_type, base_name=f"FusionBarrier_{safe_name}")
...
def _infer_num_quantizers(onnx_path):
    ...
    raise RuntimeError(
        f"could not infer num_quantizers from {onnx_path} "
        f"(audio_codes dim 2 is not a static positive integer)")
...
def parse_args():
    p = argparse.ArgumentParser(
        description="Build a TensorRT engine from the Qwen3-TTS codec ONNX")
    ...
    p.add_argument("--batch-profile", nargs=3, type=int, default=[1, 8, 32],
                   metavar=("MIN", "OPT", "MAX"))
    p.add_argument("--frames-profile", nargs=3, type=int, default=[30, 30, 30],
                   metavar=("MIN", "OPT", "MAX"))
    p.add_argument("--fp32", action="store_true",
                   help="Build pure FP32 engine (default: FP16).")
    p.add_argument("--no-fusion-barrier", action="store_true",
                   help="Skip the TRT-10.15 fusion-barrier ONNX patch "
                        "(without it the engine is wrong at dynamic batch > 1).")
    return p.parse_args()
...
def main():
    ...
    convert_to_trt(
        onnx_path, trt_path, args.trtexec_bin,
        nq=nq,
