...
"""Disaggregated TTS client: chains standalone stages over HTTP.

Reference coordinator that calls talker (stage 0) then forwards codec tokens
to code2wav (stage 1) to produce audio. Model-neutral: select any
standalone-capable TTS model via --model.
