...
if [[ $# -lt 1 ]]; then
  echo "Usage: $0 <video_path> [prompt] [output_wav]" >&2
  exit 1
fi
...
VIDEO_PATH="$1"
PROMPT="${2:-Describe the video in one single detailed spoken sentence of at least sixty words, covering the moving objects, their colors, their approximate sizes, the direction and pattern of their motion over time, the dark background, and the overall scene, and keep the answer natural but long enough to last more than ten seconds.}"
OUTPUT="${3:-minicpmo45_video_to_audio.wav}"
...
python3 - "${VIDEO_PATH}" "${MODEL}" "${PROMPT}" "${REF_AUDIO_PATH}" "${REQUEST_JSON}" <<'PY'
import base64
import json
import mimetypes
import sys
...
video_path, model, prompt, ref_audio_path, output_path = sys.argv[1:]
path = Path(video_path)
mime = mimetypes.guess_type(path.name)[0] or "video/mp4"
video_url = f"data:{mime};base64,{base64.b64encode(path.read_bytes()).decode()}"
payload = {
    "model": model,
    "messages": [
        {
