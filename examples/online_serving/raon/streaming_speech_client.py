"""WebSocket client for streaming text-input TTS with Raon-Speech.
...
def main():
    parser = argparse.ArgumentParser(description="Streaming TTS client for Raon-Speech")
    parser.add_argument(
        "--url",
    ...
    parser.add_argument(
        "--response-format",
        default="wav",
        choices=["wav", "pcm", "flac", "mp3"],
        help="Audio format (default: wav)",
    )
    parser.add_argument(
        "--stream-audio",
        action="store_true",
        help="Receive PCM chunks per sentence (requires --response-format pcm)",
    )
    ...
    parser.add_argument(
        "--ref-audio",
        default=None,
        help="Reference audio path or URL for voice cloning",
    )
    parser.add_argument(
        "--ref-text",
        default=None,
        help="Transcript of the reference audio (optional)",
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.1,
        help="Delay between words in STT simulation (seconds, default: 0.1)",
    )
    parser.add_argument(
        "--max-new-tokens",
        type=int,
    ...
    config: dict = {}
    for key in ["model", "response_format", "max_new_tokens", "ref_audio", "ref_text"]:
